...
> [Müvekkil] için günlük operasyonel düzenleyici. [RAFİNERİ] (petrol piyasası), [HALKA AÇIK İŞTİRAK] (petrokimya), [ELEKTRİK ÜRETİM İŞTİRAKİ] (elektrik üretim), [DAĞITIM İŞTİRAKİ] (doğal gaz dağıtım) — hepsi EPDK lisans + tebliğ kapsamında.
...
| **4646 Doğal Gaz Piyasası Kanunu** | Doğal gaz dağıtım ([DAĞITIM İŞTİRAKİ]) + iletim ([BORU HATTI PROJESİ]) + ticaret |
...
tr_ictihat_ara(
  ...
  chamber="D13",
  query="+EPDK +lisans +iptal +idari +ceza",
...
```
tr_mevzuat_ara(number="5015", types=["KANUN"])  # PPK
tr_mevzuat_ara(number="4646", types=["KANUN"])  # DGPK
tr_mevzuat_ara(number="6446", types=["KANUN"])  # EPK
tr_mevzuat_ara(number="4628", types=["KANUN"])  # EPDK K.
tr_mevzuat_ara(query="EPDK", types=["YONETMELIK"])
tr_mevzuat_ara(query="EPDK", types=["TEBLIGLER"])
```
