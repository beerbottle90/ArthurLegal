...
- Kaynaklar: ArthurLegal MCP (`tr_`) ✓ verify | Resmi Gazete tarama ✓
...
2. **Geçen 7 günün RG'lerini ArthurLegal MCP (`tr_`) üzerinden tara:**
   - `tr_mevzuat_ara(query="<sektör anahtar kelime>", search_in="fulltext")`
   - `tr_mevzuat_ara(query="<>", types=["CB_KARARNAME"], search_in="fulltext")` / `tr_mevzuat_ara(<filtre>, types=["CB_GENELGE"])`
...
### Sektör düzenleyici kurum kararları (ArthurLegal MCP (`tr_`)'de değil — kurum siteleri)
