...
|---|---|
| dilekce gerekcesini guclendiren Turk doktrini — DergiPark 19 hukuk dergisi (ceza → Ceza Hukuku ve Kriminoloji, idari → Idare Hukuku ve Ilimleri) — **ikincil**, dilekcede birincil kaynakla birlikte kullan | `/legal-research:karsilastirmali-doktrin` |
| uyusmazlik AZ hukukuna tabiyse akt lafzi + **yururluk statusu** | `/legal-research:az-mevzuat` |
...
description: >
  ...
  (m.272+, iki hafta), temyiz (m.286+), CMK m.141 koruma tedbiri tazminatı. Örnek senaryolar:
...
| İşlem | Süre | Dayanak |
|---|---|---|
| **İstinaf** (ilk derece hükme karşı) | **İki hafta** (gerekçeli hükmün tebliğinden) | CMK m.273 |
| **Temyiz** (BAM kararına karşı) | **İki hafta** (gerekçeli hükmün tebliğinden) | CMK m.291 |
...
```
tr_ictihat_ara(query="<suç + somut konu>",
  chamber="<ilgili Yargıtay CD, örn. 12. CD (taksirle ölüm), 15. CD, 11. CD>",
  date_from="2021-01-01")
```
Yalnızca çekileni işle: `[ArthurLegal TR — Yargıtay 12.CD — Esas/Karar — GG.AA.YYYY]`.
...
HUKUKİ NEDENLER   : TCK m.[...], CMK `[ArthurLegal TR — GG.AA.YYYY]`
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) [✓/✗]; ArthurLegal MCP (`tr_`) (CD) [✓ N / ✗]
...
```
tr_ictihat_ara(query="<konu> iptal yürütmenin durdurulması",
  chamber="13. Daire" /* EPDK/Rekabet */ veya "10. Daire"/"14. Daire" /* ÇED/çevre */,
  date_from="2022-01-01")
tr_kurum_karari_ara (kurum="rekabet") / tr_kurum_karari_ara (kurum="kik") / tr_kurum_karari_ara (kurum="kvkk")  /* kurum birincil */
```
Yalnızca çekileni dilekçeye işle: `[ArthurLegal TR — Danıştay 13.D — Esas/Karar — GG.AA.YYYY]`.
...
II. HUKUKA AYKIRILIK NEDENLERİ
  ...
  [her biri + emsal: [ArthurLegal TR — Danıştay …]]
...
HUKUKİ SEBEPLER   : 2577 İYUK, 6446 EPDK/Elektrik Piyasası K., 2872 Çevre K.,
                    ilgili yönetmelikler `[ArthurLegal TR — GG.AA.YYYY]`
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) [✓/✗]; ArthurLegal MCP (`tr_`) (Danıştay/kurum) [✓ N / ✗]
...
description: >
  ...
  dava şartı, zamanaşımı, deliller, emsal içtihat (ArthurLegal MCP (`tr_`)). Örnek senaryolar:
...
`[ArthurLegal MCP (`tr_`)]` ile teyit. Yanlış görev/yetki → görevsizlik/yetkisizlik kararı + gönderme (m.20); süre korunsa da gecikme/masraf.
...
```
tr_ictihat_ara(query="<uyuşmazlık konusu anahtar kelimeler>",
  chamber="<ilgili Yargıtay HD, örn. 11. HD/15. HD/9. HD>", date_from="2022-01-01")
```
Lehte/aleyhte eğilimi göster, dilekçeye **yalnızca çekilen** kararı işle: `[ArthurLegal TR — Yargıtay X.HD — Esas/Karar — GG.AA.YYYY]`. **Uydurma içtihat yazma.**
...
## ⚠️ İnceleyen notu
- **Kaynaklar:** ArthurLegal MCP (`tr_`) [✓ N madde / ✗]; ArthurLegal MCP (`tr_`) [✓ N emsal / ✗]
...
> 3. **Emsal güçlendir** — ArthurLegal MCP (`tr_`)'den ek karar
...
| **CMK (ceza)** | İstinaf 2 hafta (m.273) · temyiz 2 hafta (m.291) — ikisi de gerekçeli hükmün tebliğinden · itiraz 2 hafta (m.268, öğrenmeden) · KYOK itiraz 2 hafta (m.173) |
...
- **Nispi harç:** konusu para olan davalarda dava değeri üzerinden (peşin ¼) — güncel **Harçlar Kanunu (492) tarifesi** `[ArthurLegal MCP (`tr_`) / doğrulayın]`
