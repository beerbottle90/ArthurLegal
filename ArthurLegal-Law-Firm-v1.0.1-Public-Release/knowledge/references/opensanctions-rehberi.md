...
## ArthurLegal Holding için neden kritik?

Şirket'in commercial-legal'da "the thing that hurts" olarak işaretlenen yaptırım taraması, 6 farklı rejimi tek tek tarama gerektiriyordu. OpenSanctions **tek API çağrısı ile 70+ liste** veriyor.
...
Şirket için: **commercial vendor screening → `default` veya `sanctions`**, PEP'ler counterparty UBO incelemesinde alttan kontrol.

---

## API key prosedürü (Şirket için)

**Adım 1:** https://www.opensanctions.org/api/ → Sign up

**Adım 2:** İletişim bilgileri + kullanım amacı (commercial screening for ArthurLegal Holding) → ticari ekiple lisans görüşmesi
...
- **Lisans aktif** — Şirket ticari abonelik (13.05.2026'dan itibaren). Kullanım kotası ve aşım takibi OpenSanctions panelinden izlenir.
- **Veri günlük güncellenir** — gece batch; bir günlük gecikme normal.
- **Türk MASAK listesi gözüksə də zayıf kapsama** — TR rejimine özel kararlar için MASAK web sayfasına ek bakış. Yargı MCP'den de KVKK Kurulu veri ihlali kararları taranabilir.
...
| **Yargı MCP — KVKK Kurulu** | (MCP) | TR enforcement actions |
...
- **Kalibrasyon yıllık:** match skoru eşikleri Şirket risk komitesi tarafından yıllık gözden geçirilmeli
