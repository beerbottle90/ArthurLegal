...
## Mahkeme için ne işe yarar? (emsal/içtihat taraması)

| Konu | Yargı MCP'den çekilebilen |
|---|---|
| **İş davası içtihadı** | Yargıtay ilgili Hukuk Daireleri — işe iade, kıdem, fazla mesai |
| **Çevre & İSG kaza** | Yargıtay ilgili Hukuk/Ceza Daireleri + Ceza Genel Kurulu + 6331 |
| **Sermaye piyasası uyuşmazlığı** | Danıştay 13. ve 10. Daire (düzenleyici işleme itiraz) |
| **EPDK lisans iptali davası** | Danıştay 13. Daire — enerji idari yargı |
| **Rekabet Kurulu ihlal kararı + temyizi** | Rekabet Kurumu (orijinal) + Danıştay (yargısal denetim) |
| **KVKK ihlal kararı** | KVKK Kurulu — emsal idari para cezaları |
| **Transfer fiyatlandırması** | GİB özelge + Danıştay vergi daireleri |
| **KİK ihale itirazı** | Kamu ihalelerinde teklif/iptal uyuşmazlıkları |
...
Dikey entegrasyon / piyasa yapısı emsali aramasında.
...
İlişkili taraf işlemlerinde emsal fiyat denetimi emsali aramasında.
...
Kamu enerji ihaleleri emsali aramasında.
...
## Yargı MCP kullanım disiplini (mahkeme)

1. **İçtihat araştırması** (makale, tez, mütalaa için) → Yargı MCP birinci kaynak; atıf daima verbatim.
2. **İş / tazminat / ceza davaları** → ilgili Yargıtay Daireleri + Genel Kurullar.
3. **İdari/vergi davaları** → Danıştay ilgili daireleri + İDDK/VDDK.
4. **Düzenleyici kurum kararları** (EPDK/Rekabet/KVKK/KİK) → kurum kararı + Danıştay yargısal denetimi.
5. **Anayasal boyut** → AYM norm denetimi + bireysel başvuru.

Her araştırma — **kanun metni (Mevzuat MCP) + yargı yorumu (Yargı MCP)** birlikte değerlendirilmeli; bilimsel yorum, değerlendirme ve sorumluluk **yazarındır**.

> 🔴 **Akademik kullanımda:** Dipnota girecek her karar MCP'den **verbatim** çekilir ve
> `[Yargı MCP — kurum — Esas/Karar — GG.AA.YYYY]` etiketiyle işaretlenir. Çekilemeyen karar
> `[MANUEL DOĞRULAYIN]` olarak bırakılır — **esas/karar numarası asla uydurulmaz.** Uydurma
> atıf, yayın etiği yönergeleri kapsamında intihal/sahtecilik riskidir.
> → `../skills/atif-kaynak__skills.md` · `yayin-etigi-rehberi.md`
