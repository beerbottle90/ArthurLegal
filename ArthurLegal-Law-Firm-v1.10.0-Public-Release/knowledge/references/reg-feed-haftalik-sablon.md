...
### [Mevzuat adı + sayı]
**Kaynak:** RG [GG.AA.YYYY] sayı [N] [ArthurLegal TR — GG.AA.YYYY]
...
- Kaynaklar: ArthurLegal MCP (`tr_`) ✓ verify | Resmi Gazete tarama ✓ | konu süzgeci: elenen [n], belirsiz [n]
...
2. **Geçen 7 günün Resmî Gazete'lerini ArthurLegal MCP (`tr_`) üzerinden tara** (mevzuat araması RG akışını vermez; akış fihristtedir):
   - `tr_resmi_gazete_tara(konu="enerji", date_from="<pazartesi>", date_to="<pazar>")`; izlenen her konu için tekrarla (`enerji`, `rekabet`, `vergi`, `icra`). Başlığında konu adı geçmeyen kalemi de bulur.
   - `tr_resmi_gazete_tara(query="<sektör anahtar kelimesi>", date_from=…, date_to=…)`; `konu` bir ön elemedir, anahtar kelime taramasıyla tamamlanır. Dört konunun dışındaki alanlar (KVKK, iş hukuku, sermaye piyasası) yalnız `query` ile taranır.
   - Haftanın kanunları, torba kanunlar dâhil: `tr_mevzuat_ara(types=["KANUN"], rg_date_from=…, rg_date_to=…, konu="<konu>")`; `konu_kaynak: "belirsiz"` dönen kanunu `tr_mevzuat_getir` ile aç.
   - İnceleyen notuna her tarama için `konu_elenen` ve `konu_belirsiz` sayısını yaz. `konu` kapsam garantisi değildir (`mevzuat-mcp-rehberi.md` bölüm 4); bültende "bu hafta başka düzenleme yok" denmez.
3. **Sektör kurumu kararlarını ArthurLegal MCP (`tr_`) üzerinden çek:** `tr_kurum_karari_ara(kurum="epdk" | "bddk" | "spk" | "kvkk" | "rekabet" | "btk" | "gib" | "sigorta_tahkim")`; SPK için `tr_spk_bulten_icinde_ara`. Kurum sitesi web fetch yalnız bu sekiz kurumun dışındakiler (KGK, MASAK, TÜRKPATENT, bakanlıklar) içindir.
...
## [Müvekkil] için özel kaynak listesi (her hafta zorunlu)

### Birincil mevzuat kaynakları
| Kaynak | URL | Periyot | Sorumluluk |
|---|---|---|---|
| Resmi Gazete | https://www.resmigazete.gov.tr | Günlük (09:30) | Tüm grup |
| Mevzuat Bilgi Sistemi | https://www.mevzuat.gov.tr (ArthurLegal MCP (`tr_`)) | Sürekli | Hukuk |
...
|---|---|---|
| **EPDK** | https://www.epdk.gov.tr/Detay/Icerik/3-0-39-3/son-kurul-kararlari/elektrik | Petrol, doğal gaz, elektrik ([HALKA AÇIK İŞTİRAK], [RAFİNERİ], [ELEKTRİK ÜRETİM İŞTİRAKİ], dağıtım) — **kritik** |
...
- 🔴 **EPDK kurul kararları** ([HALKA AÇIK İŞTİRAK]/[RAFİNERİ]/[ELEKTRİK ÜRETİM İŞTİRAKİ] lisansları, doğal gaz pazar dengesi)
