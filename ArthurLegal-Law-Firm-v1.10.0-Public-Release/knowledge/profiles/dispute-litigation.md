...
|---|---|---|
| **ArthurLegal MCP (`tr_`)** (TR yargı kararları) | ✓ (.mcp.json'da hazır) | Manuel UYAP/Lexpera arama |
| **ArthurLegal MCP (`tr_`)** (TR mevzuat) | ✓ | mevzuat.gov.tr manuel |
| **ArthurLegal MCP (`tkgm_`)** (taşınmaz: parsel, kroki, dayanak köprüsü; bilgi amaçlı) | ✓ | Parsel Sorgu / e-Devlet manuel |
...
[Büro Adı] – Matter ID: [matter-slug] – [Müvekkil takma adı]
...
> **⚠️ İnceleyen notu**
> - **Kaynaklar:** ArthurLegal MCP (`tr_`) [✓ N karar çekildi / ✗ bağlanamadı]; ArthurLegal MCP (`tr_`) [✓ X madde / ✗]; UYAP [manuel doğrulanmalı]
...
- `[ArthurLegal TR — yargitay/danistay/aym — GG.AA.YYYY]` — yargı kararı, oturumda çekildi
- `[ArthurLegal TR — GG.AA.YYYY]` — kanun/yönetmelik metni, oturumda çekildi
...
**Güncellik tetikleyici:** Türk hukuku hızlı değişir. Cevap güncelliğe bağlıysa **ArthurLegal MCP (`tr_`)'de ilgili konuyu önce ara**.

**Müvekkilin söylediği hukuki olguyu önce doğrula:** Müvekkil bir madde no, esas/karar no, zamanaşımı tarihi, eşik tutar verirse, Mevzuat/ArthurLegal MCP (`tr_`)'ye veya verilen dosyaya karşı **önce teyit et**, sonra analize başla.
...
**Hassas karar — ön kontrol (pre-flight):** Yargı/ArthurLegal MCP (`tr_`) yanıt veriyor mu test et; vermiyorsa `Kaynaklar:` satırına yaz.
...
- **Hukuki sorun** (HMK kuralı, içtihatın yorumu, zamanaşımı) → Mevzuat/ArthurLegal MCP (`tr_`) + analiz
...
| Dava | Müvekkil takma adı | Karşı taraf takma adı | Mahkeme | Esas No | Tip | Not |
|---|---|---|---|---|---|---|
| [DOLDUR] | | | | | | |
