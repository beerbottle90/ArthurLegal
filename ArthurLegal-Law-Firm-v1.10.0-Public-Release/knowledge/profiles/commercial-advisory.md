# Commercial Advisory Practice Profile (Türk Hukuku — Büro tarafı)

*Bu dosya `/commercial-advisory:cold-start-interview` ile doldurulur.*

---

## Kim olduğumuz

`firm-profile.md` oku. Bu eklentiye özel:

**Pratik modeli:** Ticari sözleşme **YAZMA + inceleme + müzakere** — müvekkil pozisyonunda
**Aylık sözleşme hacmi:** `[DOLDUR — örn. 10-20 sözleşme/ay (yazma + inceleme karışık)]`
**Birincil yakıcı sorun:** `[DOLDUR — örn. müvekkillerin "dün lazım" baskısı + tutarlılık (her ortağın farklı şablonu) + damga vergisi tartışması]`

**Sorumlu ortak:** `[DOLDUR — örn. Yönetici Ortak]`

---

## Kim kullanıyor

**Rol:** [DOLDUR]
**Asıl avukat irtibatı:** [DOLDUR]

---

## Entegrasyonlar

| Entegrasyon | Durum | Alternatif |
|---|---|---|
| **ArthurLegal MCP (`tr_`)** (TBK + TTK + KEP + DVK + SMK) | ✓ | mevzuat.gov.tr |
| **ArthurLegal MCP (`tr_`)** (ticari uyuşmazlık emsalleri) | ✓ | UYAP |
| **ArthurLegal MCP (`tkgm_`)** (taşınmaz satışı, kira, teminat: parsel ve harç ön hesabı; bilgi amaçlı) | ✓ | Parsel Sorgu manuel |
| **iManage / SharePoint** (sözleşme arşivi) | [DOLDUR] | Drive |
| **KEP** (sözleşme teslim) | [DOLDUR] | E-posta |
| Şablon kütüphanesi (büro içi) | [DOLDUR — varsa konum] | — |

---

## Sözleşme tip envanteri (büromuzun en sık ürettiği)

| Tip | Aylık hacim | Şablon var? | Sorumlu ortak |
|---|---|---|---|
| NDA (gizlilik) | `[DOLDUR — örn. 4-6]` | ✓ | Yönetici Ortak |
| Hizmet sözleşmesi | `[DOLDUR — 3-5]` | ✓ | Yönetici Ortak |
| Tedarik / satış | `[DOLDUR — 2-4]` | ✓ | — |
| Distribütörlük / acentelik | `[DOLDUR — 1-2]` | ✓ | — |
| Lisans (yazılım/marka) | `[DOLDUR — 1-3]` | Kısmen | IP danışman |
| Franchise | `[DOLDUR — nadir]` | Kısmen | — |
| Joint venture / ortaklık | `[DOLDUR — nadir]` | Kısmen | Yönetici Ortak |
| SaaS aboneliği (müvekkil alıcı/satıcı) | `[DOLDUR — 2-4]` | ✓ | — |
| Eser sözleşmesi (inşaat/yazılım) | `[DOLDUR — 1-3]` | Kısmen | — |
| Kira (ticari) | `[DOLDUR — 2-3]` | ✓ | — |
| İhtarname | `[DOLDUR — 5-10]` | ✓ (yapısal) | — |

---

## Müvekkil pozisyonu - üç tip

Bir sözleşme matter'ında müvekkilin pozisyonunu sınıflandır:

1. **A-tarafı (güçlü):** Standart şartlarını dayatabilir; karşı yan kabul eder
2. **B-tarafı (denk):** Karşılıklı müzakere; "tipik" piyasa şartları
3. **C-tarafı (zayıf):** Karşı yanın şablonunu kabul etmek zorunda; sadece kritik maddeleri müzakere edebilir

Bu sınıflandırma **draft/review yaklaşımını belirler:**
- A-tarafı için → kendi şablonumuzu yaz, max savunma
- B-tarafı için → dengeli müzakere noktaları belirle
- C-tarafı için → kritik 3-5 madde flag, gerisini kabul önerisi

---

## Vekalet ücreti modeli — sözleşme danışmanlığı

**Standart yaklaşım:** `[DOLDUR]`
- **Tek sözleşme — basit (NDA, kısa hizmet):** Sabit `[X TL]` (~AAÜT × 0.5-1)
- **Tek sözleşme — karmaşık (JV, franchise, M&A SPA):** Saatlik veya AAÜT × 2-5
- **Aylık retainer (sürekli danışmanlık müvekkili):** Aylık sabit `[X TL]` — N saat dahil, fazla saat ekstra
- **Müzakere desteği** (sözleşme dışı saat): Saatlik

---

## Türk hukuku — sözleşme standart guardrail'lar

### TBK temel hatırlatmalar

- **TBK m. 115 — Sorumsuzluk şartı:** Ağır kusur ile sebep olunan zarar için **kesin hükümsüzdür**. NDA/sözleşmelerde "her türlü sorumluluğun reddi" maddeleri TBK 115'e takılır — yumuşat.
- **TBK m. 21 — Genel işlem koşulları:** Şaşırtıcı klozlar yazılmamış sayılır. Yeni karşı yan / tüketici karşısında dikkatli ol.
- **TBK m. 27 — Kesin hükümsüzlük:** Kanuna, ahlaka, kişilik haklarına aykırı sözleşme şartı.
- **TBK m. 169-170 — İade ve uyarma:** Sözleşme öncesi/sonrası bilgi verme yükümlülüğü.

### TTK ticari sözleşme dinamikleri

- **TTK m. 18-20 — Tacir vasfı:** Tacir, basiretli iş adamı; sözleşme yorumunda standart yüksek.
- **TTK m. 21-22 — Ticari iş + faiz:** Ticari işlerde temerrüt faizi yüksek (TBK genel faizinin üstünde)
- **TTK m. 18/3 — Yazılı şekil:** Tacirler arası bazı sözleşmelerde KEP/yazılı şekil önerilir (delil)
- **TTK m. 5/A — Zorunlu arabuluculuk:** Para alacağı ticari uyuşmazlıklarda dava şartı; bu plugin **dava değil sözleşme** üretir ama **uyuşmazlık çözüm klozu** yazarken hatırlatır.

### Damga vergisi (DVK Tablo I)

Sözleşme imzalanması → damga vergisi doğar. Tipik oranlar:
- **Sözleşme genel (bedel içeren):** **binde 9,48** (2026 — yıllık günceller)
- **Kira sözleşmesi:** binde 1,89 (bedel × 1 yıl)
- **Hizmet sözleşmesi:** binde 9,48
- **Bağışlama (ivazsız):** Vergi yok (genelde)
- **Yıllık damga tavan:** TL üst sınır var (her yıl güncellenir)

⚠️ **Damga vergisini ödemeyen taraf** sözleşme delil olarak ileri sürmek istediğinde **damga vergisinin 4 katı ceza + vergi** ödemek zorunda — yani **sözleşmeyi imzalayan herkes** kendi nüsha damgasını ödemeli. Maliye sıkı kontrol etmez ama dava açıldığında çıkar.

Detay: `references/damga-vergisi-rehberi.md`

### KEP teslim (5070 sayılı K.)

Müvekkilin KEP adresi varsa, sözleşme **KEP üzerinden** teslim edilebilir → **delil değeri yüksek** (e-tebligat).

Karşı yanın KEP adresi varsa, ihtarname **KEP üzerinden** çekilebilir — noter masrafı yok.

### Uyuşmazlık çözüm klozu seçimi

| Müvekkil tipi | Önerilen kloz |
|---|---|
| KOBİ - yurtiçi karşı yan | İstanbul mahkemeleri yetkili |
| KOBİ - yurtdışı karşı yan | **ISTAC** İstanbul Tahkim Merkezi (yerli, TR dil) |
| Büyük kurumsal - uluslararası | **ICC** Paris (TR/EN) veya ISTAC |
| Yatırımcı korumalı (BIT) | ICSID kapsamı kontrol |

Detay: `references/istac-rehberi.md`

---

## Outputs

**Sözleşme taslağı (yeni yazılan):**
```
TASLAK – İMZA BEKLİYOR – [Sorumlu Avukat] Onayı Sonrası Müvekkile Sunulacak
[Büro] – Matter: [matter-slug]
Müvekkil pozisyonu: A-tarafı / B-tarafı / C-tarafı
Karşı yan: [takma ad]
Hazırlayan: Claude – avukat incelemesi öncesi
Damga vergisi tahmini: [TL]
```

**Mevcut sözleşme inceleme notu (müvekkile gönderilecek):**
```
SAYIN MÜVEKKİL – AVUKATLIK K. m. 36 – DAHİLİ
[Büro] – Matter: [matter-slug]
İncelenen: [sözleşme tipi] – [karşı yan adı]
Sözleşme tarihi: [taslak/imza]
Bulgu sınıflandırma: 🔴 N – 🟠 N – 🟡 N – 🟢 N
```

**İhtarname:** "TASLAK – İMZA BEKLİYOR + KEP gönderim hazır" ekle.

### Bulgu severity skalası

- 🔴 **Bloklayıcı** — imzalama; TBK 115 ihlali, kanuna aykırılık, müvekkilin müzakere haklarını kaybetmesi
- 🟠 **Yüksek** — müzakere şart; tek taraflı fesih hakkı yok, tutar üst sınırı belirsiz, damga ödenmemiş
- 🟡 **Orta** — fix önerilir; yetki klozu zayıf, tahkim klozu eksik, KVKK referansı yok
- 🟢 **Düşük** — bilgi notu; tipo, dilbilgisi, format

### Atıf disiplini

- `[ArthurLegal TR — TBK/TTK m. X — GG.AA.YYYY]`
- `[ArthurLegal TR — yargitay — emsal — GG.AA.YYYY]`
- `[Müvekkil pozisyonu — A/B/C-tarafı — atanan ortak değerlendirmesi]`
- `[Damga vergisi — DVK Tablo I — 2026 oran]`

---

## Karar duruşu

Sözleşme yazımında **risk-getiri** dengesinin sahibi **müvekkil**. Avukat:
- Riski **tanımlar, ölçer, sunar** (gri alanı vurgular)
- Müvekkil "bu riski kabul ediyorum" derse, açık yazılı talimat alır + matter dosyasına kaydet
- Müvekkilin maddi/operasyonel önceliği farklı olabilir — avukat **hukukun en güvenli** yolunu önerir ama dayatmaz

**Aceleci müvekkil** (özellikle "yarın imzalanacak" durumlar):
- 🟠 hep bir flag — özellikle "tek geçişlik kabul edilebilir" tonu kaçınılan ifade
- "İmzalamadan önce 24 saat bekleme" önerisi yazılı kayıt

---

## Shared guardrails

`dispute-litigation.md` ile aynı çerçeve.

**Sözleşme özel:**
- **Hiçbir zaman kanun atıfını "hatırlayarak" yazma** — ArthurLegal MCP (`tr_`)'den çek (TBK/TTK madde no'ları sık değişir).
- **ArthurLegal MCP (`tr_`) emsal arama** — özellikle TBK 115 + sorumsuzluk şartı / aksi takdirde / aşırı yararlanma tipi davalarda yeni Yargıtay görüşü kontrol et.
- **Standart şablon kullanırken** son güncellemenin tarihi `[review]` ile kontrol et.

---

## Matter workspaces

Cross-matter OFF zorunlu. Konum: `~/.claude/plugins/config/claude-for-legal-law-firm/matters/<müvekkil-slug>__<matter-slug>/`

Sözleşme matter'ları genelde **kısa ömürlü** (imza sonrası kapanır, sadece müteakip ihtilaf doğarsa yeniden açılır). Matter kapatma zamanlamasını skill `/commercial-advisory:matter-close` ile.

---

## Seed sözleşmeler

| Sözleşme | Tip | Müvekkil | Karşı yan | Durum | Not |
|---|---|---|---|---|---|
| [DOLDUR] | | | | | |

---

*Re-run interview:* `/commercial-advisory:cold-start-interview --redo`
