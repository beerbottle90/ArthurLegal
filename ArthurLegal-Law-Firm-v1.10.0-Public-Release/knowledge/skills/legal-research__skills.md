...
> litigation / tax / advocacy / expert-opinion / criminal-defense)
> dayanagini besler.
> Aktif entite ve cikti dili `firm-profile.md`'den gelir.
...
|---|---|---|---|
| TR kanun / yönetmelik / CBK metni | ArthurLegal MCP (`tr_`) (mevzuat) | BİRİNCİL | `mevzuat-mcp-rehberi.md` |
| TR yargı / Kurul kararı | ArthurLegal MCP (`tr_`) (yargı) | BİRİNCİL | `yargi-mcp-rehberi.md` |
...
| ArthurLegal MCP (`tr_`) | Yabancı hukuk |
...
## 5. 🔒 Meslek sırrı sınırı — üç MCP de PUBLIC

e-qanun, LexScholar ve ResourceContracts **kamuya açık kaynak arama
araçlarıdır.** **Avukatlık Kanunu m. 36** (sır saklama yükümlülüğü) ve
`references/mesleki-sir-rehberi.md` bu araçlar bakımından da geçerlidir.
Bunlara **gönderilmez**:

- Müvekkil adı, dosya numarası, dosya özeti
- Gizli sözleşme taslağı veya kloz metni
- Müzakere pozisyonu, ücret/eşik bilgisi
- Karşı taraf adının somut bir dosyayla birlikte geçtiği herhangi bir ifade
- Kişisel veri (KVKK)

Arama sorgusu **soyut hukuki kavram** olmalıdır ("stabilization clause cost
recovery cap", "mücbir sebep uyarlama"), **dosya alıntısı değil**.
Bir arama, karşı tarafa veya üçüncü kişiye vekâlet ilişkisini ele verebilecek
kadar belirginse **yapma** — kavramı soyutlaştır.
...
- **Tipik tetikleyiciler (müvekkil işleri):** Azerbaycanlı karşı tarafla
  intra-group veya ticari işlemler (ham petrol tedarik, royalty, transfer
  fiyatlandırması, JV), Azerbaycan'da istihdam edilen personel için iş hukuku,
  ortak girişim yapısı, Türk müvekkil ile Azerbaycanlı taraf arasındaki
  sözleşmelerde uygulanacak hukuk seçimi.
...
- TR mevzuat / içtihat → ArthurLegal MCP (`tr_`).
...
- Bu araca müvekkil bilgisi, dosya özeti veya gizli taslak **gönderme**
  (Av. K. m. 36).
...
- Besleyen skill'ler: `commercial-legal:governing-law-review`,
  ...
  `advocacy-legal` (dilekçe dayanağı), `firm-operations:new-client-intake`.
...
- Yürürlükteki mevzuat, madde metni veya bağlayıcı karar → ArthurLegal MCP (`tr_`),
...
- Müvekkile özel sözleşme pozisyonları / büro standartları → `firm-profile.md`
  ve ilgili `profiles/*.md`.
...
- Bu araca müvekkil bilgisi veya gizli taslak **gönderme** — public literatür
  aramasıdır (Av. K. m. 36).
...
- Besleyen skill'ler: `corporate-legal`, `commercial-legal:governing-law-review`,
  ...
  `advocacy-legal` (özel/kamu/ceza dilekçe gerekçesi),
  `expert-opinion:uzman-mutalaasi` (HMK m. 293 mütalaa dayanağı),
  `criminal-defense` (ceza doktrini).
...
- Müvekkile özel pozisyonlar / statik büro bağlamı → `firm-profile.md`,
  `profiles/*.md`.
...
- Besleyen skill'ler: `commercial-legal` (sözleşme incelemesi),
  ...
  `contract-drafting:redline-contract`, `expert-opinion:uzman-mutalaasi`.
