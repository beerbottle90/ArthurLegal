...
|---|---|
| mutalaanin akademik dayanagi — Turk ve karsilastirmali ogreti; **hakemlilik uc durumlu** (`true`/`false`/`null`), ABD law review'lari ogrenci editorludur ve hakemli sayilmaz | `/legal-research:karsilastirmali-doktrin` |
| sozlesmesel bir teknik meselede imzali emsal kloz metni | `/legal-research:sozlesme-emsali` |
...
```
- Metodoloji + standart (TS/IEC/şebeke yönetmeliği/EPDK tebliği/YEKDEM hesap esasları) atıfla; `[ArthurLegal MCP (`tr_`) / doğrulayın]`.
...
## Emsal / dayanak (ArthurLegal MCP (`tr_`))
```
tr_ictihat_ara(query="+bilirkişi +raporu +eksik +inceleme +<teknik +konu>",
  chamber="<ilgili HD/CD>", date_from="2021-01-01")
```
...
- **Kaynaklar:** Mevzuat/standart [✓/✗]; ArthurLegal MCP (`tr_`) emsal [✓ N / ✗]
...
IV. GEREKÇELİ ANALİZ
   ...
   [Hukuki: ilgili mevzuat `[ArthurLegal MCP (`tr_`)]` + yerleşik içtihat `[ArthurLegal MCP (`tr_`)]`
...
- **Atıf disiplini:** içtihat/madde **ArthurLegal MCP (`tr_`)'den çekilir**; doktrin atfı doğrulanır. **Uydurma kaynak = mütalaanın çürütülmesi.**

## Kaynak (ArthurLegal MCP (`tr_`))
```
tr_mevzuat_ara / tr_mevzuat_getir   # dayanak madde metni
tr_ictihat_ara(query="<hukuki sorun>", chamber="<ilgili daire>")  # yerleşik içtihat
tr_aym_ara                  # AYM (anayasal boyut varsa)
```
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) [✓ N / ✗]; ArthurLegal MCP (`tr_`) [✓ N emsal / ✗]; doktrin [✓/doğrulanmalı]
...
> 4. **Emsal/doktrin güçlendir** — ArthurLegal MCP (`tr_`)
> 5. **Başka**
