...
**Version:** 1.10.0 | **Updated:** 2026-09-23
...
| ArthurLegal MCP | `https://arthurlegal-mcp.fly.dev/mcp` — one connector, Türkiye + 14 jurisdictions + land registry, 121 tools, no auth (Step 4) |
...
One endpoint, 16 backends, 121 tools. Türkiye is inside this connector under the `tr_`
prefix (23 tools) and the Turkish land registry under `tkgm_` (17 tools) — no separate Turkish-law or
land-registry connector is needed. The first live land-registry query in a chat shows a one-time consent card;
see `knowledge/references/tapu-kadastro-rehberi.md`.

| Prefix | Coverage |
|---|---|
| `tr_` | 🇹🇷 Türkiye — Yargıtay, Danıştay, regional and local courts, KYB (Bedesten) · Constitutional Court · Uyuşmazlık Mahkemesi · 12 legislation types with article tree and gerekçe · Official Gazette · 8 regulators (Rekabet, EPDK, SPK, BDDK, KVKK, BTK, GİB, Sigorta Tahkim) · a 19,498-document semantic archive |
...
| `tkgm_` | 🇹🇷 Land registry and cadastre — live parcels from TKGM Parsel Sorgu (block/parcel, coordinates, place names), parcel report, scaled sketch, legal-basis bridge; at most 30 requests a minute, one consent card per chat |
...
This is version **1.10.0**. See `CHANGELOG.md` for what changed.

Upgrading from 1.9.1: paste `SYSTEM_PROMPT.md` into Custom Instructions again (sections 5 to 8: the `tkgm_` land-registry tools, Türkiye 0.5.0). Add `knowledge/references/tapu-kadastro-rehberi.md` to Project knowledge and replace `knowledge/references/yargi-mcp-rehberi.md` and `knowledge/references/mevzuat-mcp-rehberi.md`. Do not overwrite the `firm-profile.md` and `profiles/` files you have filled in; add the ArthurLegal Tapu (`tkgm_`) line by hand if you want it. The ArthurLegal local installer (2.1.0) refreshes the package files itself.
