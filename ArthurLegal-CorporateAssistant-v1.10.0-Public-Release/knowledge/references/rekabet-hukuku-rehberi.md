> **Durum:** ✅ Açık — ArthurLegal MCP (`tr_`) `kurum_karari_ara (kurum="rekabet")` + mevzuat araçları; rekabet.gov.tr WebFetch. Bu rehber 4054 sayılı Kanun'un esas/usul çatısını ve sektörel/örnek senaryoları derler. Bağlantılı skill'ler: `administrative-legal:rekabet-kurulu-itiraz`, `corporate-legal:merger-notification`, `energy-finance:ma-diligence-energy`.
...
## 6. Kaynak erişimi (ArthurLegal MCP (`tr_`))

```
# Rekabet Kurulu kararı arama (kartel, hakim durum, birleşme emsali)
tr_kurum_karari_ara(kurum="rekabet", query="<konu — örn. elektrik toptan bilgi paylaşımı>")
# 4054 ve tebliğ metinleri
tr_mevzuat_ara(number="4054")   # + Tebliğ 2010/4, 2002/2
```
Atıf: `[Rekabet Kurulu — karar no/tarih — GG.AA.YYYY]` (MCP'den çekildiyse `[ArthurLegal TR — rekabet — ...]`); mevzuat → `[ArthurLegal TR — GG.AA.YYYY]`. Çekmediğin karara atıf yapma.
...
*Son güncelleme: 25.06.2026 — Enerji sektörü (üretim + toptan satış + M&A) rekabet bağlamı; 2022 birleşme ciro eşikleri ve uzlaşma/de minimis (2021) dâhil. Güncel eşik/oranları ArthurLegal MCP (`tr_`)'den teyit edin.*
