# İSG Davaları Rehberi – [İşletme Yeri] Rafineri & Petrokimya Bağlamı

> [ŞİRKET ADI] için yakıcı dava kategorisi. [İşletme alanındaki] ölümlü/ciddi yaralanma kazalarında üçlü-paralel risk: cezai, tazminat, idari.
...
3. **Asliye Ceza Mahkemesi'nde dava** — [İşletme Yeri] veya Karşıyaka (yer)
...
ArthurLegal MCP (`tr_`) ile teyit:
```
tr_ictihat_ara(
  courts=["YARGITAYKARARI"],
  query="+taksirle +öldürme +iş +kazası +rafineri +yönetici +sorumluluk",
  chamber="C12",  # iş kazası temyizi
...
2. [ŞİRKET ADI]'a tebliğ → 60 gün Danıştay
3. **Lisans askıya alma = üretim durur** → büyük gelir kaybı

## [İşletme Yeri]-özel notlar
...
3. **24-72 saat:** Olay raporu inceleme, stratejik karar, KAP açıklama ([HALKA AÇIK İŞTİRAK])

## ArthurLegal MCP (`tr_`) — temel maddeler

```
tr_mevzuat_ara(number="6331", types=["KANUN"])  # İSG K.
tr_mevzuat_ara(number="6098", types=["KANUN"])  # TBK
tr_mevzuat_ara(number="5237", types=["KANUN"])  # TCK
tr_mevzuat_ara(number="2872", types=["KANUN"])  # Çevre K.
tr_mevzuat_ara(number="4857", types=["KANUN"])  # İş K.
tr_mevzuat_ara(number="5510", types=["KANUN"])  # SGK K.
```
