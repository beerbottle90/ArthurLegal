# IP Advisory Practice Profile (Türk Hukuku — Büro tarafı)

*Bu dosya `/ip-advisory:cold-start-interview` ile doldurulur.*

---

## Kim olduğumuz

`firm-profile.md` oku. Bu eklentiye özel:

**Pratik modeli:** Fikri sınai haklar — **marka + patent + tasarım + telif + alan adı**
**Aktif tescil portföyü:** `[DOLDUR — örn. müvekkillerimiz adına aktif 50-150 marka tescili]`
**Birincil yakıcı sorun:** `[DOLDUR — örn. taklit ürün (UDRP + ihtarname) yoğunluğu + YİDK itirazlarında uzun süre + Madrid Protokolü tercüme maliyetleri]`

**Sorumlu ortak:** `[DOLDUR — örn. Kıdemli Ortak C — IP uzmanlığı + marka vekili lisansı]`
**Marka vekili lisansı:** `[DOLDUR — büromuzun marka vekili olarak yetkili avukatlar / dış marka vekili firma adı]`

---

## Müvekkil profili

| Müvekkil tipi | Tipik talep |
|---|---|
| KOBİ — yeni marka | Marka clearance + tescil (TR + Madrid) |
| KOBİ — mevcut marka portföyü | Yenileme, ihtilaf yönetimi |
| Kreatif/yazılım stüdyosu | Telif (5846), yazılım lisansı, açık kaynak uyum |
| E-ticaret marka | UDRP domain takedown, ihtarname, TÜRKPATENT itiraz |
| Patent başvurucu | TR patent (genelde Türkiye'de PATENT BAŞVURUSU; **incelemeli patent başvuru süreci pahalı + uzun**) |
| Tasarım hak sahibi | Tasarım tescili, taklit takip |
| Marka ihlali davacı | İhtarname → dava (Asliye FSH Mh. veya Asliye Hukuk Mh. — büyük şehirlerde) |

---

## Entegrasyonlar

| Entegrasyon | Durum | Alternatif |
|---|---|---|
| **ArthurLegal MCP (`tr_`)** (6769 SMK, 5846 FSEK, 5651) | ✓ | mevzuat.gov.tr |
| **ArthurLegal MCP (`tr_`)** (Yargıtay 11. HD — SMK içtihadı) | ✓ | UYAP |
| **TÜRKPATENT online** (marka arama, EPATS) | Manuel | turkpatent.gov.tr |
| **EUIPO / WIPO Madrid** | Manuel | euipo.europa.eu / wipo.int |
| **WIPO UDRP** | Manuel | wipo.int/amc |
| **TR-ID alan adı** (.tr) | TR-NIC | nic.tr |

---

## Vekalet ücreti modeli — IP

**Standart yaklaşım (büromuzun tercihi):**

| İşlem | Standart ücret |
|---|---|
| Marka clearance + ön araştırma | `[DOLDUR — örn. 5.000-10.000 TL]` |
| Marka tescil başvurusu (TR) | `[DOLDUR — örn. 8.000-15.000 TL + TÜRKPATENT harcı]` |
| Madrid Protokolü başvuru | `[DOLDUR — örn. 25.000-50.000 TL + WIPO harcı]` |
| YİDK itiraz / cevap | `[DOLDUR — örn. 10.000-20.000 TL]` |
| Marka yenileme | `[DOLDUR — örn. 5.000-8.000 TL]` |
| Patent başvuru — incelemeli | `[DOLDUR — saatlik + harç]` |
| Tasarım tescil | `[DOLDUR — örn. 5.000-10.000 TL/tasarım]` |
| İhtarname | `[DOLDUR — örn. AAÜT × 1-2]` |
| Marka ihlali davası | `[DOLDUR — saatlik veya AAÜT × katsayı + başarı bonusu]` |
| UDRP domain takedown | `[DOLDUR — örn. 8.000-15.000 TL + WIPO/ICANN harcı]` |
| 5651 sayılı K. takedown (içerik kaldırma) | `[DOLDUR — örn. 3.000-5.000 TL + hosting ihtarname]` |

---

## Türk hukuku — IP temel matris

### 6769 sayılı SMK (Sınai Mülkiyet Kanunu)

| Konu | Madde | Pratik notu |
|---|---|---|
| **Marka tescili** | m. 4-22 | Başvuru → yayım → itiraz (2 ay) → tescil; toplam 12-18 ay |
| **Mutlak red sebepleri** | m. 5 | Ayırt edicilik yokluğu, tanımlayıcılık, kamu düzeni, vd. |
| **Nispi red sebepleri** | m. 6 | Benzer marka, ticaret unvanı, telif, vd. — **itiraz üzerine** |
| **Marka iptal davası** | m. 25-26 | TÜRKPATENT'e veya FSH Mh.'ne (sınırlı) |
| **Marka hükümsüzlük** | m. 25 | FSH Mh.'ne dava |
| **YİDK itiraz** | m. 20-21 | Mütalaa veya karara karşı 2 ay içinde |
| **YİDK karar iptali davası** | m. 23 | FSH Mh.'ne karara karşı 2 ay |
| **Tasarım tescili** | m. 55-81 | Yenilik + ayırt edicilik; başvuru → yayım → tescil (~6 ay) |
| **Patent — incelemeli** | m. 88-114 | TÜRKPATENT veya EPO (Avrupa); inceleme 3-5 yıl |
| **Patent — incelemesiz (faydalı model)** | m. 115-118 | Daha hızlı, daha zayıf koruma |
| **Marka ihlali** | m. 7 / 29-32 | Tazminat + ihlalin men'i + yedek imha |
| **Zamanaşımı** | m. 157 | **2 yıl (öğrenme)** / **10 yıl (eylem)** |

### 5846 sayılı FSEK (Fikir ve Sanat Eserleri K.)

- Telif hakkı **otomatik** doğar — tescil değil, **eser olma** şartı (özgünlük + somutluk)
- Tescil **isteğe bağlı** (FSEK m. 13-A — Telif Hakları Genel Müdürlüğü)
- Süre: yazar yaşam + 70 yıl
- Bilgisayar yazılımı = eser (FSEK m. 2/1)
- Açık kaynak lisansı **FSEK altında geçerli** ama lisans şartları üst hukuk

### 5651 sayılı İnternet K.

- Hosting'e ihtarname → 24 saat içinde kaldırma yükümlülüğü (m. 9)
- Sulh ceza hakimi kararı → erişimin engellenmesi
- **Hata düzeltme + cevap hakkı** (5187 sayılı Basın K. m. 14)

---

## Marka clearance akışı

1. **TÜRKPATENT marka arama** — sınıf bazlı (Nice sınıflandırması)
2. **Fonetik arama** — benzer ses
3. **Görsel arama** — özellikle figüratif markalar
4. **Ticaret unvanı arama** — MERSİS + TTSG
5. **EUIPO arama** (AB marka)
6. **Madrid Sistem arama** (uluslararası)
7. **Alan adı + sosyal medya** — domain + handle
8. **Telif kontrolü** — popüler eser/karakter çakışması
9. **Tanınmış marka durumu** — TÜRKPATENT tanınmış marka listesi + Paris Sözleşmesi m. 6bis

Detay: `references/turkpatent-rehberi.md` + `references/smk-rehberi.md`

---

## Eskalasyon ve onay matrisi

| Karar | Onay yetkisi |
|---|---|
| Marka clearance + tescil başvurusu | Atanan ortak |
| YİDK itiraz hazırlama | Atanan ortak |
| YİDK karar iptal davası | Atanan ortak + müvekkil yazılı |
| Marka ihlali davası açma | Atanan ortak + Yönetici Ortak + müvekkil yazılı |
| **Tanınmış marka müvekkili — büyük taklit operasyonu** | Yönetici Ortak + Ortaklar Kurulu |
| **Açık kaynak ihlali iddia** | Atanan ortak + dış uzman (gerekirse) |
| UDRP başvuru | Atanan ortak |
| Patent başvuru — incelemeli | **Atanan ortak + patent vekili (dış)** (büromuz patent vekili yoksa) |

---

## Outputs

**Marka clearance raporu:**
```
SAYIN MÜVEKKİL – AVUKATLIK K. m. 36 – DAHİLİ
[Büro] – Matter: [matter-slug]
Aranan marka: "[marka adı]" – Sınıf(lar): [Nice X, Y]
Müvekkil: [kod adı]
Sonuç: ✓ TEMİZ / ⚠️ ŞÜPHELİ / ⛔ ÇAKIŞMA RİSKİ
[N benzer marka bulundu — risk analizi ektedir]
```

**TÜRKPATENT başvuru paketi:** Başvuru formu + güç belgesi (vekaletname) + sınıf listesi + ücret hesabı.

**YİDK itiraz dilekçesi:** "TASLAK – İMZA BEKLİYOR" ekle.

**İhtarname:** "TASLAK – İMZA + KEP TESLİM HAZIR" ekle.

### Atıf disiplini

- `[ArthurLegal TR — 6769 SMK m. X — GG.AA.YYYY]`
- `[ArthurLegal TR — yargitay 11. HD — GG.AA.YYYY]`
- `[TÜRKPATENT — başvuru/tescil no — GG.AA.YYYY]`
- `[WIPO Madrid — IR no — GG.AA.YYYY]`
- `[EUIPO — EUTM no — GG.AA.YYYY]`
- `[Nice Sınıflandırma — 12. baskı (2025)]` (yıllık günceller)

---

## Karar duruşu

IP işlemleri **uzun vadeli koruma** — bir markayı kaybetmek 10 yıllık yatırımı kaybetmek olabilir. **Konservatif yaklaşım** baseline:
- Şüpheli benzerlik → **uyarı ver, müvekkil karar versin**
- Tanınmış marka çakışması → **HER ZAMAN** dur, başka isim öner

**Müvekkil "ben kullanırım, dava açılırsa açılır" derse:**
- Yazılı feragat al (riskini bildiğini yazılı belge)
- Matter notlarına kaydet
- Yine de tavsiyemiz net: "tescil reddine yol açar, hukuken risk yüksek"

---

## Shared guardrails

`dispute-litigation.md` ile aynı çerçeve.

**IP özel:**
- **Nice Sınıflandırma yıllık güncellenir** — 2025 baskı, 2026 baskı kontrol et (ArthurLegal MCP (`tr_`)'ye değil, WIPO/TÜRKPATENT sitesine bakılır)
- **Madrid Protokolü ülkeleri** — 130+ ülke; müvekkilin hedef pazarına göre liste değişir
- **Açık kaynak lisans uyumu** (GPL, MIT, Apache, vd.) — yazılım müvekkillerinde özel dikkat
- **UDRP** — sadece **kötü niyet + müvekkilin haklı menfaat sahibi olmaması** üçlü test

---

## Matter workspaces

Cross-matter OFF zorunlu. IP matter'ları **çok uzun ömürlü** (10+ yıl yenileme takibi). Matter `tescil → yenileme N1, N2, N3` halinde devam eder.

---

## Seed portföy

| Marka/Patent/Tasarım | Müvekkil | Tip | Tescil no | Yenileme tarihi | Not |
|---|---|---|---|---|---|
| [DOLDUR] | | | | | |

---

*Re-run interview:* `/ip-advisory:cold-start-interview --redo`
