...
> **Bu dosya kimin için:** Claude'un, kullanıcının Arthur Mask hakkındaki sorularını (kurulum, günlük kullanım, sorun giderme, sınırlar) doğru cevaplaması ve `arthur_mask_*` araçlarını doğru kullanması için. Avukata yönelik adım adım anlatım paket kökündeki `ARTHUR-MASK.md` dosyasındadır.
...
Arthur Mask, yalnız kullanıcının bilgisayarında çalışan bir Windows programıdır. Avukat bir belgeyi Arthur Mask'in yerel tarayıcı arayüzüne bırakır; belgedeki kişisel veriler `{{KİŞİ-01}}`, `{{ŞİRKET-02}}`, `{{TCKN-01}}` gibi etiketlerle değiştirilir. Gerçek değerler bilgisayarda, her dosya (matter) için ayrı ve şifreli bir kasada kalır. Claude, Claude Desktop üzerinden yalnız maskeli metni alır. Claude'un cevabı yine bilgisayarda gerçek adlara geri çevrilir ve Word veya UDF olarak açılır.
...
**İnceleme ekranı.** Tespit olasılığa dayalıdır. Program emin olmadığı tespitleri avukatın onayına sunar; avukat her biri için "Maskele" veya "Açık bırak" der ve maskeli kopyayı göndermeden önce görür.
...
**"Claude'a giden" paneli.** Claude'a gönderilen her yanıt maskeli hâliyle kayda geçer; avukat Claude'un tam olarak ne gördüğünü sonradan kontrol edebilir.
...
Konum: `Belgeler\Arthur Mask\<dosya adı>`. Maskeli kopyalar 30 gün sonra kendiliğinden silinir; gerçek adlara çevrilmiş cevaplar ve kasa kalır. Büronun saklama ve imha politikası bu klasöre de uygulanır.
...
3. Sorumluluk avukattadır: KVKK yükümlülükleri, Avukatlık Kanunu m. 36'daki sır saklama yükümlülüğü, ticari sır ve gizlilik sözleşmeleri (NDA) Arthur Mask kullanılsa da avukat ve büro bakımından devam eder. Maskeli metnin Claude'a gönderilmesinin hukuki değerlendirmesi (yurt dışına aktarım dâhil) avukata aittir.
...
5. Etiket hiçbir araştırma sorgusuna konmaz (`tr_` araçları, diğer MCP'ler, web). Hukuki soru soyut terimlerle araştırılır ("kira bedelinin uyarlanması", "işçinin haklı nedenle feshi").
...
9. Kullanıcı tanımlanabilir müvekkil veya karşı taraf kişisel verisini doğrudan sohbete yapıştırır ya da eklerse, sohbet başına en çok bir kez, işi durdurmadan kısa bir hatırlatma yapılır: belgeler Arthur Mask ile (Claude Desktop, Windows) paylaşılmadan önce bilgisayarda maskelenebilir. Reddedilmez, ders verilmez.
...
İlgili: `mesleki-sir-rehberi.md` (Av. K. m. 36), `pii-redaksiyon-rehberi.md` (maskeleme disiplini ve ölçüm).
