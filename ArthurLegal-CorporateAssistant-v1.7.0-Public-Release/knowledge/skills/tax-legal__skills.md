...
|---|---|
| AZ Vergi Mecellesi, intra-group transfer fiyatlandirmasi — akt + statu | `/legal-research:az-mevzuat` |
| transfer pricing ve uluslararasi vergi ogretisi — **ikincil**; ozelge ve Danistay karari yerine gecmez | `/legal-research:karsilastirmali-doktrin` |
...
- [HALKA AÇIK İŞTİRAK] KDV iade büyüklüğü (yıllık tahmin)
- [RAFİNERİ] ÖTV mahsubu hacmi
- [ANA ORTAK] intra-group işlem hacmi (TP risk)
...
1. **ArthurLegal MCP (`tr_`)** — test: `tr_kurum_karari_ara(kurum="gib", query="transfer fiyatlandırma")` → cevap geliyor mu?
2. **ArthurLegal MCP (`tr_`)** — test: `tr_mevzuat_ara(number="213", types=["KANUN"])` → VUK dönüyor mu?
3. **iManage** — test
...
> 1. Konuda zaten var olan bir özelge var mı? → ArthurLegal MCP (`tr_`)'den ara
...
### 2. ArthurLegal MCP (`tr_`) — benzer özelge tarama
...
Şirketimiz **[ŞİRKET ADI] A.Ş.** [vergi kimlik no, sicil no, adres] olarak,
...
### 4. ArthurLegal MCP (`tr_`) — Danıştay vergi içtihatı
...
tr_ictihat_ara(
  ...
  query="<konu> mukteza GİB",
  date_from="2022-01-01"
)
```

### 5. [ŞİRKET ADI]-özel notlar

- **[HALKA AÇIK İŞTİRAK] ilgili özelge** — KAP açıklama tetikleyici olabilir (büyük mali etki)
- **[BORU HATTI PROJESİ] işlem özelgesi** — hükümetlerarası anlaşma + GİB ortak görüşü
- **[ANA ORTAK] intra-group** — özelge talep edersek Maliye'nin dikkati çekiyor, risk
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) (benzer özelge [N]); ArthurLegal MCP (`tr_`) (madde [N])
- **Olumlu/olumsuz cevap olasılığı:** [tahmin]
- **Süre tahmini:** [6-9 ay]
- **Reviewer karar bekleyen:** N adet

## Mevcut özelge taraması (ArthurLegal MCP (`tr_`)'den)
...
description: >
  KDV iadesi ([HALKA AÇIK İŞTİRAK] ihracat) ve ÖTV mahsubu/iadesi ([RAFİNERİ] akaryakıt
...
İki ana akış:
1. **[HALKA AÇIK İŞTİRAK] KDV iadesi** — petrokimya ihracatı sonucu yüksek tutarlı KDV iade talebi
2. **[RAFİNERİ] ÖTV mahsubu/iadesi** — akaryakıt ihracatı sonucu ÖTV mahsubu
...
### [ŞİRKET ADI] [HALKA AÇIK İŞTİRAK] için özel
...
### Süreç
1. **[RAFİNERİ] ihracat:** Akaryakıt ihracatı (gemiyle, boru hattıyla) → ihracat beyannamesi
...
> A. [HALKA AÇIK İŞTİRAK] KDV iadesi
> B. [RAFİNERİ] ÖTV mahsubu/iadesi
> C. Diğer ([DAĞITIM İŞTİRAKİ] KDV, vs.)
...
### 3. Dokümantasyon checklist ([HALKA AÇIK İŞTİRAK] KDV iade)
...
### 4. Dokümantasyon checklist ([RAFİNERİ] ÖTV mahsubu)
...
### 5. ArthurLegal MCP (`tr_`) — emsal arama
...
tr_ictihat_ara(
  ...
  chamber="D3",
  query="+KDV +iade +ihracat +petrokimya +bekletme",
  date_from="2022-01-01"
)
...
tr_ictihat_ara(
  courts=["DANISTAYKARAR"],
  chamber="D3",
  query="+ÖTV +mahsup +akaryakıt +ihracat +istisna",
  date_from="2022-01-01"
)
...
- [ ] **[HALKA AÇIK İŞTİRAK] yıllık KDV iadesi > [X] TL** → KAP açıklama riski + IR Başkanlığı
- [ ] **Müşteri portföyü:** İlişkili kişi satışları ayrı raporlanmış mı (TP)
- [ ] **EPDK lisans uyumu:** Lisans kapsamında ihracat yapıldı mı ([RAFİNERİ])
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) (Danıştay [N], GİB özelge [N])
...
**One question I'd ask:** [örn. "[HALKA AÇIK İŞTİRAK] KAP'a iade beklentisini açıklamış mı? Beklenti karşılanamazsa yatırımcı algısı etkilenebilir."]
```
...
**[ŞİRKET ADI] örnek:**
- [HALKA AÇIK İŞTİRAK] KDV iadesi reddi → **3. Daire**
- [RAFİNERİ] ÖTV mahsubu reddi → **3. Daire**
...
B. [Argüman 2] — [Emsal Danıştay kararı: `[ArthurLegal MCP (`tr_`) — danistay — esas/karar — GG.AA.YYYY]`]
...
### 4. ArthurLegal MCP (`tr_`) — emsal Danıştay kararları
...
- **[HALKA AÇIK İŞTİRAK] ilgili** → KAP açıklama (önemli dava — SPK Tebliğ II-15.1)
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) (Danıştay [N] emsal); ArthurLegal MCP (`tr_`) (VUK/KDVK m. [N])
...
## Emsal Danıştay kararları
[ArthurLegal MCP (`tr_`)'den çıkan tablolar]

## Sonraki adımlar

> 1. **Dış vergi avukatı brief'i** — `/litigation-legal:outside-counsel-brief` (vergi davası için)
> 2. **CFO + Başkan eskalasyon paketi**
> 3. **KAP açıklama metni** ([HALKA AÇIK İŞTİRAK] ilgili büyük tarhiyat)
> 4. **Uzlaşma paralel değerlendirme** — `/tax-legal:uzlasma-eval` (alternatif yol)
> 5. **Daha fazla bilgi** — [...]

**One question I'd ask:** [örn. "Aynı daire son 12 ayda benzer [ŞİRKET ADI]/sektör davalarında nasıl karar verdi? Maliye lehine eğilim varsa strateji değişir."]
```
...
description: >
  Intra-group işlem fiyatlandırması analizi: [İLİŞKİLİ TARAF] ile ham petrol alımı, royalty,
...
**[ŞİRKET ADI]-özel risk üçlüsü:**
1. **Ham petrol alımı ([İLİŞKİLİ TARAF] → [RAFİNERİ])** — yüksek tutar, fiyat dalgalanması (Brent benchmark)
2. **Royalty / lisans / marka** ([ANA ORTAK] → [ŞİRKET ADI] + bağlı şirketler)
3. **Yönetim hizmeti, IT, mali danışmanlık** ([ANA ORTAK] → [ŞİRKET ADI])
...
|---|---|---|---|---|
| [ŞİRKET ADI] ([ANA ORTAK MERKEZİ]) | Ham petrol satın alım | [...] | USD | sürekli |
| [ŞİRKET ADI] | Royalty | [...] | USD | yıllık |
| [ŞİRKET ADI] | Yönetim hizmeti | [...] | USD | yıllık |
...
| Yöntem | Uygulama | [ŞİRKET ADI] örneği |
|---|---|---|
...
| **Kâr Bölüşümü (Profit Split)** | Toplam kârın paylaşımı | Karmaşık entegre işlemler ([BORU HATTI PROJESİ] gibi) |
| **İşlemsel Net Kâr Marjı (TNMM)** | Net kâr marjı karşılaştırma | Yaygın — bilgi yetersiz ise |

**[ŞİRKET ADI] önerisi:** Ham petrol için **KKF** (Brent + Azeri Light premium) en güçlü; royalty için **TNMM**; hizmet için **Maliyet Artı**.
...
**Sorun:** Bu veri tabanlarına [ŞİRKET ADI]'ın **lisans abonelikleri** var mı? — `[DOLDUR]`. Yoksa Büyük 4 firma genelde abone ve raporu hazırlar.

### 4. ArthurLegal MCP (`tr_`) — GİB özelge + Danıştay emsali

```
tr_kurum_karari_ara(
  kurum="gib",
  query="transfer fiyatlandırma ham petrol ilişkili"
)

tr_ictihat_ara(
  courts=["DANISTAYKARAR"],
  chamber="D4",  # Kurumlar vergisi
  query="+örtülü +kazanç +dağıtımı +transfer +pricing",
  date_from="2022-01-01"
)
```
...
- **BEPS Action 13** — Country-by-Country Reporting (CbCR — [ŞİRKET ADI] dahil)

Atıf: `[OECD TPG — Chapter X — 2022]`, `[BEPS Action — sayı]`.

### 6. [ANA ORTAK]-özel kontroller

- [ ] **Azerbaycan-Türkiye Çifte Vergilendirmeyi Önleme Anlaşması** (1994) — varsayılan tutar ve emsal
- [ ] **[BORU HATTI PROJESİ] işlem rejimi** — hükümetlerarası anlaşma kapsamında özel tarife
- [ ] **[HALKA AÇIK İŞTİRAK] KAP açıklama** — büyük tarhiyat halinde özel durum
...
- **Local File** — [ŞİRKET ADI] spesifik: işlemler, yöntem, emsal analizi, fonksiyonel analiz
- **Country-by-Country Reporting (CbCR)** — global [ŞİRKET ADI] ([ANA ORTAK MERKEZİ] ana şirket tarafından)
...
- **Risk seviyesi:** 🔴/🟠/🟡/🟢
- **Kaynaklar:** ArthurLegal MCP (`tr_`) (GİB özelge [N], Danıştay [N]); OECD TPG referansları
...
## [ANA ORTAK]-özel notlar
[BIT anlaşması, [BORU HATTI PROJESİ] rejimi, KAP, MASAK, yaptırım]
...
- [ ] Master File hazır
- [ ] Local File hazır
- [ ] CbCR (global [ŞİRKET ADI] tarafından)
...
**One question I'd ask:** [örn. "Maliye RIA programında [ŞİRKET ADI]'ın yer aldığına dair bir gösterge var mı? Yıllık denetim seçim olasılığını bilmek strateji değiştirir."]
```
...
description: >
  ...
  Danıştay dava alternatifiyle maliyet karşılaştırma; [ŞİRKET ADI] onay zinciri.
user-invocable: true
---
...
**Kazanma olasılığı tahmini:** ArthurLegal MCP (`tr_`)'den emsal kararlara bakarak.
...
- TP tarhiyatı ([ANA ORTAK] ilgili) → CEO + Başkan
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) (Danıştay [N] emsal); ArthurLegal MCP (`tr_`)
- **KAP:** [N/A / gerekli]
- **Reviewer bekleyen:** N adet

## Maliye iddiası vs. [ŞİRKET ADI] pozisyonu
[tablo]
...
- [ ] **[HALKA AÇIK İŞTİRAK] ilgili tarhiyat** → KAP açıklama kontrolü
- [ ] **TP konusu** → [ANA ORTAK] ile koordinasyon
- [ ] **VUK m. 359 kaçakçılık iddiası** → derhal ceza avukatı + CEO
- [ ] **[RAFİNERİ] ÖTV / [HALKA AÇIK İŞTİRAK] KDV iade reddi** → kaza-bazlı dava + uzlaşma kombinasyonu
...
**One question I'd ask:** [örn. "Maliye denetçisi geçmişte [ŞİRKET ADI]'a karşı agresif mi yumuşak mı yorum yaptı? İlişki sürdürmek karar değiştirebilir."]
```
