...
>
> **Gizlilik:** uc MCP de PUBLIC arama aracidir — gizli taslak, muzakere
> pozisyonu veya kisisel veri **gonderilmez**.
...
ArthurLegal MCP (`tr_`)'den HMK ve görevli mahkeme yönetmeliklerini çek, atıf et: `[ArthurLegal TR — GG.AA.YYYY]`.
...
### 6. ArthurLegal MCP (`tr_`) — emsal içtihat ara

Karşı tarafın iddiasının hukuki dayanağı ne ise, **ArthurLegal MCP (`tr_`)'den** o konudaki son 2-3 yıl emsal Yargıtay/Danıştay kararını çek:

```
tr_ictihat_ara(
  query="<konu anahtar kelimeleri>",
  chamber="<daire kodu, ör. H9, C12, D13, HGK, IDDK>",
  date_from="2023-01-01"
)
```

Karşı tarafa olumlu mu, bize olumlu mu eğilim? Bir kaç emsal göster, atıfla: `[ArthurLegal MCP (`tr_`) — yargitay — GG.AA.YYYY]`.
...
- [ ] **halka açık iştirak ilgili mi?** → KAP açıklama kontrolü gerekli mi? (`references/kap-esirket-webfetch-rehberi.md`)
- [ ] **[TESİS LOKASYONU] İSG kazası mı?** → `/litigation-legal:isg-incident-response` skill'i ÇOK DAHA UYGUN
...
- [ ] **Basına intikal etmiş veya etme riski?** → İletişim Başkanlığı ([İletişim Başkanlığı]) koordinasyon
...
## ⚠️ İnceleyen notu
- **Kaynaklar:** ArthurLegal MCP (`tr_`) [✓ N emsal / ✗]; ArthurLegal MCP (`tr_`) [✓ X madde / ✗]
...
**Önerilen:** [mahkeme] — HMK m. [X] kapsamında. `[ArthurLegal TR — GG.AA.YYYY]`
**Alternatif/itiraz mümkün:** [varsa]

## Husumet

- Aktif husumet: [değerlendirme]
- Pasif husumet: [Müvekkil] mi, yoksa [bağlı şirket]? `[review]`

## Zamanaşımı

- **Hesaplanan zamanaşımı tarihi:** GG.AA.YYYY (kalan: N gün)
- **Dayanak:** [TBK/TTK/İş K. madde] — `[ArthurLegal TR — GG.AA.YYYY]`
- **Risk:** [yorum]
...
## Emsal içtihat (ArthurLegal MCP (`tr_`))

| Karar | Tarih | Daire | Eğilim | Bizim için |
|---|---|---|---|---|
| [esas/karar no] | [...] | [...] | bize/aleyhe/karma | [yorum] `[ArthurLegal MCP (`tr_`) — yargitay — GG.AA.YYYY]` |

## sektöre-özel kontroller

- [ ] [halka açık iştirak] KAP: [N/A veya açıklanan]
- [ ] [TESİS LOKASYONU] İSG: [N/A veya yönlendir]
...
1. **ArthurLegal MCP (`tr_`)** — test sorgu: `tr_ictihat_ara(courts=["YARGITAYKARARI"], query="+işe +iade", date_from="2025-01-01")` → cevap geliyor mu?
2. **ArthurLegal MCP (`tr_`)** — test sorgu: `tr_mevzuat_ara(number="6100", types=["KANUN"])` → HMK dönüyor mu?
...
|---|---|---|
| ArthurLegal MCP (`tr_`) | ✅ N karar döndü | Aktif |
| ArthurLegal MCP (`tr_`) | ✅ HMK 6100 bulundu | Aktif |
| iManage | ⚠️/❌ | [neden] |
```
...
- **sektöre-özel:** [TESİS LOKASYONU] tesisinde keşif → güvenlik prosedürleri (yangın eğitimi, koruyucu ekipman); EPDK lisans kapsamında izinler

## ArthurLegal MCP (`tr_`) — emsal aramaları

Delil sunumu prosedürlerinde benzer dava emsallerini ArthurLegal MCP (`tr_`)'den çek:

```
tr_ictihat_ara(
  courts=["YARGITAYKARARI"],
  query="+HMK +220 +belge +ibraz +aleyhe +karine",
  chamber="HGK",
...
```
tr_ictihat_ara(
  query="+kayıtlı +elektronik +posta +KEP +delil",
...
- **Mevzuat referansları:** HMK m. [...], TCK m. 281, TTK m. 64, KVKK m. 7 `[ArthurLegal TR — GG.AA.YYYY]`
- **Emsal:** [N karar] `[ArthurLegal MCP (`tr_`) — yargitay — GG.AA.YYYY]`
...
İnceleyen notu standardı `practice profile → ## Outputs` bölümündeki [Müvekkil] formatına göre.
...
**Taraflar:** | **Karşı vekil:** | **[Müvekkil] vekili:** | **Konu özeti:**
...
|---|---|---|
| [...] | [...] | `[ArthurLegal MCP (`tr_`) / ArthurLegal MCP (`tr_`) — GG.AA.YYYY]` |
...
```
tr_ictihat_ara(
  query="<dava konusu> bilirkişi rapor itiraz",
  chamber="<daire kodu, ör. H9, C12, D13, HGK, IDDK>",
  date_from="2023-01-01"
)
```
...
description: >
  ...
  büyük endüstriyel tesis gibi yüksek riskli tesislerde ölümlü/ciddi kaza
...
|---|---|---|
| **[halka açık iştirak]** | ✅ **EVET (BIST: [ticker])** | **DOĞRUDAN — SPK Tebliğ II-15.1 m. 5 "önemli olay"** |
| **[büyük enerji tesisi] A.Ş.** | ❌ Hayır ([Müvekkil] Rafineri A.Ş.'ye bağlı) | Doğrudan değil — ancak **[HALKA AÇIK İŞTİRAK] ile entegre operasyon etkisi varsa [HALKA AÇIK İŞTİRAK] üzerinden dolaylı KAP** |
| **[Müvekkil]** (holding) | ❌ Hayır | Doğrudan değil |
| **[Müvekkil] Depolama / Terminal / Enerji Ticaret** | ❌ Hayır | Doğrudan değil |
| **[enerji üretim tesisi]** | ❌ Hayır | EPDK bildirim, KAP değil |
| **[DAĞITIM İŞTİRAKİ]** | ❌ Hayır | KAP değil; etki [HALKA AÇIK İŞTİRAK]'e yansıyorsa dolaylı |
...
2. Olay **[RAFİNERİ]/[Müvekkil]-bağlı şirkette** ama **[HALKA AÇIK İŞTİRAK] üretim/satış zincirini etkiliyor** mu? (örn. [RAFİNERİ]'dan etilen tedariki kesilirse [HALKA AÇIK İŞTİRAK] üretimini durdurur) → **[HALKA AÇIK İŞTİRAK] üzerinden dolaylı KAP değerlendirmesi** — Yatırımcı İlişkileri ile materyalite testi
3. Olay **[RAFİNERİ]/[Müvekkil]-bağlı** ama **[HALKA AÇIK İŞTİRAK]'i etkilemiyor** → KAP açıklama **N/A**, sadece iç eskalasyon + basın koordinasyonu

**[Müvekkil] uygulama:** "[HALKA AÇIK İŞTİRAK] etkisi yok ise KAP N/A" kararı **yazılı dokümante edilmeli** (sonradan SPK denetiminde gerekçe olarak gösterilir). IR Başkanlığı + Hukuk ortak imzalı not.
...
> Bu yeni bir İSG/çevre olayı. Dosya açayım mı? Slug: `isg-YYYY-MM-DD-[tesis]`). Cevap: evet/hayır/manuel slug.
...
| **KAP açıklama metni (halka açık iştirak ilgili ise)** | Ayrı bir çıktı; bu skill başlatır, son halini SPK uzman avukat onayı sonrası |
...
| 15-30 dk | İş Birimi Başkanı ([İş Birimi Başkanı] — [enerji tesisi]; veya [İş Birimi Başkanı] — doğal gaz) | Telefon |
| 30-60 dk | **Hukuk Direktörü** (Group GC) → **Hukuk Başkanı ([CLCO / GC])** → **CEO ([CEO])** | Telefon zinciri |
...
- [ ] **İSG/iş kazası uzmanı dış vekil** derhal aranır — `practice profile → outside counsel paneli → İSG / iş kazası` ([TESİS LOKASYONU] için yerel firma kritik; İstanbul'dan sadece koordinasyon)
...
tr_ictihat_ara(
  courts=["YARGITAYKARARI"],
  query="+iş +kazası +ölümlü +6331 +işveren +sorumluluk",
  chamber="H9",  # iş kazası tazminat
  date_from="2023-01-01"
)

tr_ictihat_ara(
  query="+taksirle +öldürme +rafineri +patlama +yönetici +sorumluluk",
  date_from="2020-01-01"
...
tr_ictihat_ara(
  ...
  chamber="D10",
  query="+çevre +kirletme +idari +para +cezası +iptali",
  date_from="2023-01-01"
)
```
...
Üst yönetim eskalasyon, basın koordinasyon, KAP açıklaması GENELDE gerekmez (halka açık iştirak ilgili değilse).
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) [✓ N emsal / not yet]; ArthurLegal MCP (`tr_`) [✓ X madde]
...
| Ceza | [...] | [...] | [...] |
| Çevre | [...] | [...] | [...] |
| KAP/SPK | [...] (halka açık iştirak ilgili ise) | [...] | [...] |
...
description: >
  ...
  bütçe, raporlama), iç hukuk pozisyonu, kritik notlar, escalation eşikleri. [Müvekkil]
...
[Müvekkil] - Sayın [vekil adı / büro] dikkatine
Esas no: [...] | Taraflar: [...] | Mahkeme: [...]
Tarih: GG.AA.YYYY | Briefer: [in-house Counsel adı]
[Müvekkil] iç onay: [Hukuk Direktörü/Başkan onayı geçtiyse "Onaylı" + tarih]
```
...
[Müvekkil]'da **tüm davalar dış vekille yürütülüyor**. In-house ekibinin işi dış vekile **standardize edilmiş, eksiksiz, tek-istek paketleri** vermek. İyi brief = kısa iş ücreti + iyi cevap + daha az takip e-posta.
...
```markdown
**Dosya bilgisi:**
- [Müvekkil] dosya no: [iç sistem]
- Esas no (varsa): [mahkemeden]
- Taraflar: [Davacı/Davalı [Müvekkil] mı?]
- Mahkeme/yargı yeri: [...]
- Konuya tarihi: [...]
- Cevap/itiraz süresi: [tarih + kalan gün]

**[Müvekkil] organizasyon bilgisi:**
- İlgili iş birimi: [Rafineri & Petrokimya / Doğal Gaz & Enerji / Portföy]
- İlgili tüzel kişi: [[Müvekkil] / [HALKA AÇIK İŞTİRAK] / [büyük enerji tesisi] / [Müvekkil] Enerji Ticaret / ...]
- İçeride dosya sahibi (in-house Counsel): [isim, iletişim]
- Eskalasyon zinciri: [Counsel → Direktör → ... → CEO]

**KAP / yatırımcı bilgi (halka açık iştirak ilgili ise):**
...
### İç araştırma (ArthurLegal MCP (`tr_`)'den çekilenler)
- [Karar 1] `[ArthurLegal MCP (`tr_`) — daire — GG.AA.YYYY]` — [özet/eğilim]
- [Karar 2] — [...]
```
...
```markdown
## Raporlama disiplini ([Müvekkil] standardı)
...
- **Çıkar çatışması:** Yeni bir [Müvekkil] davası ile çakışma riski olursa derhal bildirin
...
- **Basın takibi:** Bu davanın basına yansıma olasılığı [yüksek/orta/düşük]. Basından bir gelişme olursa İletişim Başkanlığı ([İletişim Başkanlığı]) ile koordineli — kendi başınıza basına açıklama yapmayın.
```
...
## 3. İç hukuk pozisyonu
[argümanlar + ArthurLegal MCP (`tr_`)'den çekilen emsal]
...
description: >
  ...
  damga vergisi), risk-benefit matriksi, emsal sulh tutarları (ArthurLegal MCP (`tr_`)),
  eskalasyon kararı, müzakere paketi. [Müvekkil] sulh yetki matriksine entegre.
user-invocable: true
---
...
- **Emsal Yargıtay/Danıştay kararları** (ArthurLegal MCP (`tr_`)):
```
tr_ictihat_ara(
  query="<dava konusu>",
  chamber="<daire kodu, ör. H9, C12, D13, HGK, IDDK>",
...
### 5. [Müvekkil] sulh yetki matriksi kontrolü
...
- [ ] halka açık iştirak ilgili mi? → KAP açıklama hazırlığı
- [ ] [TESİS LOKASYONU] İSG kazası mı? → otomatik Başkan
...
Anlaşma maddeleri:
1. [Müvekkil], Mağdur'a [tutar] TL ödemeyi kabul eder; ödeme tarihi [tarih].
2. Mağdur, ödemenin alınmasının ardından [Müvekkil]'a karşı bu konudaki tüm taleplerinden feragat ettiğini ve ibra ettiğini beyan eder.
...
**Emsal kararlar:** `[ArthurLegal MCP (`tr_`) — yargitay/danistay — GG.AA.YYYY]`
