...
|---|---|
| idare hukuku ogretisi — DergiPark *Idare Hukuku ve Ilimleri* dergisi router tarafindan secilir; **ikincil** | `/legal-research:karsilastirmali-doktrin` |
...
description: >
  ...
  sorunlu olduğunda — büyük endüstriyel tesis için kritik. Çevre K. 2872 + ÇED Yön.
...
**büyük endüstriyel tesis için bu kritik:** [enerji tesisi / halka açık iştirak] ÇED yenileme + kapasite artırımı dosyalarında **30 günlük süreye** sıkı uyum şart. Geç kalma = hak düşürücü süre = yatırım blokesi.
...
**büyük endüstriyel tesis sürekli ÇED yenileme + değişiklik süreçlerinde:**
- [büyük enerji tesisi] kapasite artırımı
- halka açık iştirak yeni üretim hattı
- [Müvekkil terminali] kapasite genişleme
- [enerji santral] revizyonları
...
tr_ictihat_ara(
  ...
  chamber="D14",
  query="+ÇED +ret +iptal +rafineri +petrokimya",
...
tr_ictihat_ara(
  courts=["DANISTAYKARAR"],
  chamber="D10",
  query="+çevre +etki +değerlendirme",
  date_from="2022-01-01"
)
```
...
- **Sektör emsali yatırımların ÇED onayı** (benzer [enerji tesisi], neden bu reddedildi?)
...
**Compliance ekibi:** [Uyum Direktörü] + [Compliance Manager] çevre uyum bilgisi sahipdir — onlarla brief.

**İletişim Başkanlığı:** [İletişim Başkanlığı] — basın ve sosyal medya etkili olacaksa.

**İlgili iş birimi:** [İş Birimi Başkanı] — operasyonel etki bilgisi.
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) (Danıştay 14./10. [N]); ArthurLegal MCP (`tr_`) (Çevre K. 2872 + ÇED Yön.)
...
> 4. **Compliance brief** — [Uyum Direktörü]/[Compliance Manager] ile teknik koordinasyon
> 5. **Daha fazla bilgi** — [...]
...
- Çevre Bakanlığı + il müdürlüğü koordinasyon ([TESİS LOKASYONU] için)
- KVKK Kurumu ile ilişki
- KİK ihale itirazları sıklık
- SPK / BDDK (halka açık iştirak ilgili) ilişki

### Batch 5 — Halka açık ve özel kontroller
- halka açık iştirak ilgili tüm idari konular için KAP koordinasyon
...
1. **ArthurLegal MCP (`tr_`)** — test: `tr_ictihat_ara(courts=["DANISTAYKARAR"], chamber="D13", query="+EPDK +lisans")` → cevap geliyor mu?
2. **ArthurLegal MCP (`tr_`)** — test: `tr_mevzuat_ara(number="2577", types=["KANUN"])` → İYUK dönüyor mu?
3. **iManage** + diğerleri
...
description: >
  EPDK Kurul'a proaktif görüş/talep sunma. Dava ÖNCESİ çözüm kanalı. [Müvekkil]'ın
  EPDK ile yoğun dialog modelinde temel araç. Kurul kararı çıkmadan önce yorum,
  Tebliğ taslağına görüş, lisans şartı yorumu, görüş başvurusu. [Uyum Direktörü]
  (Compliance Direktörü) + [Compliance Manager] (Regulatory Compliance Manager) ile
...
büro pratiğinde **dava açmadan önce EPDK ile dialog kanalı** kullanılır:
...
- **Sorun:** [örn. "Yeni tebliğde belirlenen rapor formatı [Müvekkil] için uygulanamaz çünkü..."]
- **Etki:** [operasyonel + mali]
- **Önerimiz:** [somut alternatif]

### 2. Dayanak araştırma

- **ArthurLegal MCP (`tr_`)** — ilgili kanun/tebliğ + EPDK Kanunu (4628 + 5015 + 4646 + 6446)
- **ArthurLegal MCP (`tr_`) — Danıştay 13. Daire** — benzer konularda Danıştay yorumu (EPDK aleyhine olan emsaller bizim için anchor)
...
tr_ictihat_ara(
  courts=["DANISTAYKARAR"],
  chamber="D13",
  query="+EPDK +lisans +şartı +yorumu +emsal",
  date_from="2022-01-01"
)
```
...
Konu: [Tebliğ/Karar adı] hakkında [Müvekkil] görüşü
...
[Düzenlemenin neyin etkilediği, [Müvekkil]/[enerji tesisi / halka açık iştirak]/[enerji üretim tesisi] üzerindeki
...
Bazı görüşler **sektör koalisyonu** halinde (Türkiye Petrolleri, Tüpraş, Aygaz, vs. ile birlikte) sunulursa daha güçlü. [Müvekkil] + diğerleri ortak görüş = sektör temsili.
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) (Danıştay 13. [N]); ArthurLegal MCP (`tr_`)
...
- **Genel kural (İYUK m. 32):** Davalı idarenin bulunduğu yer İdare Mahkemesi
  ...
  - [tesis yeri] Belediyesi imar kararları = **İzmir İdare Mahkemesi** ([TESİS LOKASYONU] bağlı)
...
### 6. ArthurLegal MCP (`tr_`) — emsal arama

```
# Bireysel idari işlem için Danıştay temyiz emsali
tr_ictihat_ara(
  courts=["DANISTAYKARAR"],
  chamber="D13",  # EPDK/Rekabet/BDDK/SPK/KİK için
  query="<konu> iptal yürütmenin durdurulması",
  date_from="2022-01-01"
)
...
tr_ictihat_ara(
  courts=["DANISTAYKARAR"],
  chamber="D10",
  query="+çevre +ÇED +idari +para +cezası",
  date_from="2022-01-01"
)
...
tr_kurum_karari_ara(kurum="rekabet", query="<konu>")
kurum_karari_ara(kurum="kik", keywords="<konu>")
tr_kurum_karari_ara(kurum="kvkk", query="<konu>")
```
...
C. **Sebep yokluğu** [argüman + emsal Danıştay kararı: `[ArthurLegal MCP (`tr_`) — danistay — daire — esas/karar — GG.AA.YYYY]`]
...
### 8. sektöre-özel kontroller

- [ ] **Tutar > 10M TL** veya **stratejik önem** → CEO + Başkan eskalasyon
- [ ] **halka açık iştirak ilgili** → KAP açıklama kontrolü + IR Başkanlığı
- [ ] **EPDK lisans askıya alma riski** → derhal Başkan + CEO + iş birimi
- [ ] **[TESİS LOKASYONU] İSG/çevre olayı sonrası idari ceza** → litigation-legal `isg-incident-response` ile koordine (üçlü-paralel dava)
- [ ] **Üst yönetim ismi var** → CEO + YK
- [ ] **Basın takibi** → İletişim Başkanlığı ([İletişim Başkanlığı]) koordinasyon
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) (Danıştay [N] + kurum [N]); ArthurLegal MCP (`tr_`)
...
[ArthurLegal MCP (`tr_`)'den çekilen tablolar]
...
> 3. **KAP açıklama metni** (halka açık iştirak ilgili büyük etki)
> 4. **EPDK proaktif görüş** alternatifi düşünme — `/administrative-legal:epdk-proaktif-gorus` (dava açmadan önce dialog şansı)
> 5. **Daha fazla bilgi** — [...]

**One question I'd ask:** [örn. "Bu daire son 12 ayda benzer [Müvekkil]/sektör davalarında bir eğilim gösterdi mi? Davayı kazanma ihtimalini değiştirir."]
```
...
> - Çevre Bakanlığı (2872 ÇK) — [tesis yeri] çevre ihlali
...
| Çevre İl Müdürlüğü ([tesis yeri]) | İzmir İdare Mah. |
...
tr_ictihat_ara(
  ...
  query="<kurum> idari para cezası iptal",
  date_from="2022-01-01"
)
```
...
### 5. sektöre-özel kontroller

- [ ] **Ceza tutarı > 10M TL** → CLCO + CEO eskalasyon
- [ ] **[TESİS LOKASYONU] İSG/çevre olayı kaynaklı** → litigation-legal `isg-incident-response` ile koordine
- [ ] **halka açık iştirak ilgili** → KAP açıklama kontrolü
- [ ] **EPDK lisans askıya alma riskini içeriyor mu** → derhal CLCO + iş birimi başkanı
- [ ] **Basın etkili** → İletişim Başkanlığı
- [ ] **Compliance koordinasyon** (KVKK → [DPO / Veri Koruma Sorumlusu]; Rekabet/EPDK → [Uyum Direktörü]/[Compliance Manager])
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) (Danıştay [N])
- **Reviewer bekleyen:** N adet
...
> 4. **KAP açıklama metni** (halka açık iştirak ilgili)
> 5. **Tehir-i icra paralel** (tahsil başladıysa)

**One question I'd ask:** [örn. "Bu ceza kurumun yıllık ortalamasının üstünde mi? Cezalar genelde benzer ölçekte mi yoksa [Müvekkil]'a karşı agresif mi?"]
```
...
[Müvekkil] ilgili daireler (CLAUDE.md'de detay):
...
tr_aym_ara(query="<konu> mülkiyet hakkı ihlali")
```

## sektöre-özel

- Temyiz/istinaf karar dava açma yetkisi — practice profile eskalasyon matriksinde
- halka açık iştirak ilgili → KAP açıklama
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) [N], ArthurLegal MCP (`tr_`)
- **KAP açıklama:** [N/A / hazırlanıyor]
- **Reviewer bekleyen:** N adet
...
description: >
  ...
  taraf [Müvekkil] (büyük enerji ihaleleri).
user-invocable: true
---
...
İhale sıklığı:
- **Devlet ihalelerine teklif veren** (Elektrik üretim teşvik ihaleleri — benzer büyük)
...
İstekli/İstekli adayı: [Müvekkil] (veya bağlı şirket)
İhaleyi yapan idare: [...]
İhale konusu: [...]
İhale tarih + numara: [...]
...
### 4. ArthurLegal MCP (`tr_`)

```
# KİK ArthurLegal MCP'de YOK (EKAP v2 API 500 döndürüyor); yalnız isteğe bağlı TR Legal MCP kuruluysa:
kurum_karari_ara(
  kurum="kik",
  keywords="<konu>",
...
tr_ictihat_ara(
  courts=["DANISTAYKARAR"],
  chamber="D13",
  query="+KİK +karar +iptal +ihale",
  date_from="2022-01-01"
)
```

### 5. müvekkil koordinasyon

- **İhale Komisyonu** (iç) — iş birimi tarafında
- **Senior Legal Counsel** — KİK şikayet + dava hazırlık
- **Direktör ([Hukuk Direktörü])** — onay
- **CLCO** — büyük ihale / stratejik etki
- **Compliance ([Uyum Direktörü])** — kamu ihale uyumu
...
- **İhale değeri:** [TL]
- **Kaynaklar:** ArthurLegal MCP (`tr_`) (KİK [N], Danıştay 13. [N]); ArthurLegal MCP (`tr_`) (4734 KİK K.)
...
description: >
  ...
  Daire. [Müvekkil] vertical entegrasyon (rafineri + petrokimya + terminal + dağıtım)
...
- Pazar paylaşımı (intra-group [Ana ortak / ilişkili taraf] ile)
...
tr_ictihat_ara(
  courts=["DANISTAYKARAR"],
  chamber="D13",
  query="+Rekabet +Kurulu +karar +iptal",
  date_from="2022-01-01"
)
```
...
- **Compliance ([Uyum Direktörü])** — rekabet uyumu kapsamı
...
- **Kaynaklar:** ArthurLegal MCP (`tr_`) (Rekabet Kurulu [N], Danıştay 13. [N])
- **CEO bilgisi:** [✓/✗]
- **KAP açıklama:** [N/A / hazırlanıyor]
- **Reviewer bekleyen:** N adet
