...
- **İş davaları (4857 + HMK):** İşyerinin bulunduğu yer (Aliağa, Yalova, vs.)
...
- **Pasif husumet:** Davalı olarak [Müvekkil] mi, yoksa bir bağlı şirket (Petkim, STAR, [Müvekkil] Enerji Ticaret, vs.)?
- **Tüzel kişilik perdesi:** Karşı taraf ana ortaktan değil de bağlı şirketten talepte bulunabilir mi?
- **Aliağa ÖEB özelliği:** Aliağa ÖEB içindeki tesisler ayrı tüzel kişiler — husumet doğru tespit edilmeli
...
- [ ] **Aliağa İSG kazası mı?** → `/litigation-legal:isg-incident-response` skill'i ÇOK DAHA UYGUN
...
- [ ] **TANAP / hükümetlerarası anlaşma alanı?** → Dış hukuk panel + üst yönetim
...
- **Tutardan bağımsız otomatik eskalasyon listesi** (Petkim, üst yönetim isim, basın, EPDK lisans riski, yaptırım, vs.)
...
- Halka açık şirket varsa (örn. Petkim) — KAP açıklama disiplini
...
| **Bilirkişi** | m. 266-287 | Teknik/uzmanlık gerektiren mesele | Rafineri/petkim teknik konular, çevre etkisi, vergi hesabı |
| **Keşif** | m. 288-292 | Mahkemenin yerinde inceleme | Aliağa tesisi kaza sonrası keşif |
...
- **sektöre-özel:** Aliağa tesisinde keşif → güvenlik prosedürleri (yangın eğitimi, koruyucu ekipman); EPDK lisans kapsamında izinler
...
## ⚠️ KAP açıklama tetik haritası — STAR vs Petkim ayrımı (KRİTİK)
...
| **[büyük enerji tesisi] A.Ş.** | ❌ Hayır ([Müvekkil] Rafineri A.Ş.'ye bağlı) | Doğrudan değil — ancak **Petkim ile entegre operasyon etkisi varsa Petkim üzerinden dolaylı KAP** |
...
| **Bursagaz / Kayserigaz (devir öncesi/sonrası)** | ❌ Hayır | KAP değil; devir aşamasında etki Petkim'e yansıyorsa dolaylı |

**Karar matrisi:**

1. Olay **Petkim tesisinde** veya **Petkim faaliyetiyle doğrudan ilgili** mi? → **KAP açıklama derhal** (Yatırımcı İlişkileri + Petkim YK Sekreterya)
2. Olay **STAR/[Müvekkil]-bağlı şirkette** ama **Petkim üretim/satış zincirini etkiliyor** mu? (örn. STAR'dan etilen tedariki kesilirse Petkim üretimini durdurur) → **Petkim üzerinden dolaylı KAP değerlendirmesi** — Yatırımcı İlişkileri ile materyalite testi
3. Olay **STAR/[Müvekkil]-bağlı** ama **Petkim'i etkilemiyor** → KAP açıklama **N/A**, sadece iç eskalasyon + basın koordinasyonu

**[Müvekkil] uygulama:** "Petkim etkisi yok ise KAP N/A" kararı **yazılı dokümante edilmeli** (sonradan SPK denetiminde gerekçe olarak gösterilir). IR Başkanlığı + Hukuk ortak imzalı not.
...
| **KAP** (halka açık iştirak ilgili önemli olay) | DERHAL özel durum açıklaması | Petkim | KAP üzerinden, SPK Tebliğ II-15.1 |
| **TANAP** (TANAP altyapı etkilendiyse) | 24 saat | İşletmeci | TANAP işletme sözleşmesi |
...
| 90 dk - 2 saat | Compliance + Yatırımcı İlişkileri (Petkim ise) | E-posta + telefon |

`company-profile.md` anahtar kişiler tablosunu kontrol et — son atamalar değişebilir.

#### 4. Dış vekil mobilizasyonu

- [ ] **İSG/iş kazası uzmanı dış vekil** derhal aranır — `practice profile → outside counsel paneli → İSG / iş kazası` (Aliağa için yerel firma kritik; İstanbul'dan sadece koordinasyon)
...
- [ ] **Sermaye piyasası uzmanı dış vekil** (Petkim ise)
...
- [ ] **KAP açıklaması** (Petkim ise) — finansal etki + üretim duruş açıklanır
...
> 3. **KAP açıklama metni** — Petkim için
...
- **KAP'ı atlama** (Petkim ise) — SPK yaptırımı + yatırımcı davası açar.
...
- İlgili tüzel kişi: [[Müvekkil] / Petkim / [büyük enerji tesisi] / [Müvekkil] Enerji Ticaret / ...]
...
- **KAP / yatırımcı etkili gelişme:** halka açık iştirak ilgili tüm önemli gelişmeler derhal (Petkim dosyalarında)
```
...
- **Ana ortak [Ana ortak / ilişkili taraf] ile koordinasyon:** Stratejik dosyalarda Bakü ile bilgi paylaşımı yapılmadan önce onay alınmalı (intra-group işlemler).
...
- [ ] **Yatırımcı algısı (Petkim):** Yüksek tutarlı sulh KAP açıklaması gerektirebilir, hisse fiyatına etki.
- [ ] **Emsal oluşturma riski:** Bu sulhten sonra benzer davalar açılır mı? (örn. Aliağa'da bir çalışana 5M TL sulh = diğer çalışan davaları için baseline)
...
- [ ] Aliağa İSG kazası mı? → otomatik Başkan
...
- Yatırımcı (Petkim): [değerlendirme + KAP gereği]
