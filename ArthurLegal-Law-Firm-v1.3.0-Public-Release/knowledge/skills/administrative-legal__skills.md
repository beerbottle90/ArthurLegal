...
- Çevre Bakanlığı + il müdürlüğü koordinasyon (Aliağa için)
...
- **Genel kural (İYUK m. 32):** Davalı idarenin bulunduğu yer İdare Mahkemesi
  ...
  - [tesis yeri] Belediyesi imar kararları = **İzmir İdare Mahkemesi** (Aliağa bağlı)
...
- [ ] **Aliağa İSG/çevre olayı sonrası idari ceza** → litigation-legal `isg-incident-response` ile koordine (üçlü-paralel dava)
- [ ] **Üst yönetim ismi var** → CEO + YK
- [ ] **Basın takibi** → İletişim Başkanlığı ([İletişim Başkanlığı]) koordinasyon
- [ ] **TANAP / hükümetlerarası anlaşma alanı** → dış uzman + üst yönetim
...
- [ ] **Aliağa İSG/çevre olayı kaynaklı** → litigation-legal `isg-incident-response` ile koordine
...
description: >
  ...
  Daire. [Müvekkil] vertical entegrasyon (rafineri + petkim + terminal + dağıtım)
...
- **[büyük enerji tesisi] (üst kademe)** + **Petkim (orta kademe petrokimya)** + **[Müvekkil terminali] (lojistik)** + **[Müvekkil] Enerji Ticaret (dağıtım)** = baştan sona zincir kontrolü
- **Aliağa ÖEB** içinde dikey entegre yapı
- **EPC tedarik zinciri** (yan kuruluşlardan tedarik)
- **Doğal gaz dağıtım eskisi** (Bursagaz/Kayserigaz devir öncesi pazar payı yüksek)
...
> 4. **KAP açıklama** (Petkim büyük etki)
