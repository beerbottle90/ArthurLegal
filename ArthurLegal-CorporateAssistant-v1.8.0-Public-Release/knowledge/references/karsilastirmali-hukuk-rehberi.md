...
### MCP sunucuları (WebFetch değil — ayrı connector)

| MCP | Endpoint | Auth | Rol | Rehber |
|---|---|---|---|---|
| **ArthurLegal MCP (`tr_`)** | `arthurlegal-mcp.fly.dev/mcp` | OAuth (WorkOS) | TR mevzuat + yargı — **BİRİNCİL** | `mevzuat-mcp-rehberi.md`, `yargi-mcp-rehberi.md` |
| **OpenCaseLaw.ch** | `mcp.opencaselaw.ch/sse` | Yok | CH içtihat + Fedlex — **BİRİNCİL (CH)** | `switzerland-caselaw-rehberi.md` |
| **ArthurLegal MCP** | `arthurlegal-mcp.fly.dev/mcp` | Yok | On yargı çevresi tek uçta: AZ mevzuatı + yürürlük statüsü, AT/DE/NL/PL/ES/FI/IE mevzuat ve içtihadı, hukuk doktrini, imzalı sözleşme emsali | `eqanun-mcp-rehberi.md`, `at-ris-mcp-rehberi.md`, `de-eli-mcp-rehberi.md`, `nl-rechtspraak-mcp-rehberi.md`, `pl-sejm-mcp-rehberi.md`, `es-boe-mcp-rehberi.md`, `fi-finlex-mcp-rehberi.md`, `ie-statutebook-mcp-rehberi.md`, `lex-scholar-rehberi.md`, `resourcecontracts-rehberi.md` |

> **ArthurLegal MCP auth'suzdur** — adresi bilen herkes tüm araçları çağırabilir;
> müvekkil adı veya gizli taslak gönderme. Araç adları yargı çevresi önekli
> (`az_` `at_` `de_` `nl_` `pl_` `es_` `fi_` `ie_` `scholar_` `contracts_`);
> `status` aracı her yargı çevresinin indeks kapsamını verir. Bağlantı koparsa
> ilgili rehberin yedek yoluna düş.
>
> ⚠️ **Aynı ortamda iki connector aynı araç adını taşımamalı.** Bir literatür
> sunucusuyla yaşanan `search_articles` çakışması istemcinin şemaları
> karıştırıp `search_articles_2` üretmesine ve çağrıların 400 ile düşmesine yol
> açtı. LexScholar araçları bu yüzden `search_legal_scholarship` gibi
> ayrıştırılmış adlar taşır. Yeni bir connector eklerken araç adlarını mevcut
> connector'larla karşılaştır.

---

## ⏱️ 100 saniye kuralı — her araç çağrısı için bağlayıcı

Platform her araç çağrısını **100 saniyede iptal eder** ve iptal edilen çağrı
**hiçbir şey döndürmez** — kısmî sonuç bile gelmez. Bu, kapsamı doğrudan etkiler:

- **Sorguyu dar tut:** tek yargı çevresi veya tek mesele; makul `limit`;
  tam metni `offset` / sayfa penceresiyle al.
...
- İptal olursa **aynı sorguyu tekrarlama** — böl, ve **hangi kısmın
  kapsanmadığını kullanıcıya söyle**. Kısmi cevabı tam gibi sunma.
...
içinde döner; genel web-arama sohbeti gezinmek zorundadır ve bu limite rutin
olarak takılır. Ölçüm (25.07.2026): Güney Afrika, Almanya ve Birleşik Krallık'ı
kapsayan bir karşılaştırmalı soru özel araçtan **1 saniyenin altında** üç yargı
çevresinden de isabetli makale döndürdü; **aynı soru** genel web-arama
sohbetine gönderildiğinde **iki kez 100 saniyede iptal edildi**. Genel web
aramasına yalnız hiçbir özel araç o kaynağı kapsamıyorsa git.

**Araçların NE TUTMADIĞINI bil.** LexScholar hukuk *hakkında* literatür
döndürür — kanunun veya kararın **resmî metnini değil**. Güney Afrika, Almanya
ve pek çok yabancı birincil kaynak için connector **yoktur**; bu durumda
"resmî metin çekilmedi" de ve literatürü **şerh/yorum** olarak atıfla — resmî
kaynağa bakılmış gibi **ima etme**.

---

## 1. Temel ilke — birincil yargı çevresi Türkiye

[ŞİRKET ADI] paketinin **birincil yargı çevresi Türkiye Cumhuriyeti'dir.**
...
|---|---|---|
| Türk kanunu, yönetmeliği, CB kararnamesi | ArthurLegal MCP (`tr_`) (mevzuat) | `mevzuat-mcp-rehberi.md` |
| Türk/idari yargı kararı, Kurul kararı | ArthurLegal MCP (`tr_`) (yargı) | `yargi-mcp-rehberi.md` |
...
| **Azerbaycan mevzuatı + yürürlük statüsü** 🆕 | **e-qanun MCP** | `eqanun-mcp-rehberi.md` |
| **Yabancı / karşılaştırmalı akademik doktrin** 🆕 | **LexScholar MCP** | `lex-scholar-rehberi.md` |
| **Türk akademik doktrini (DergiPark)** 🆕 | **LexScholar MCP** (19 TR hukuk dergisi) | `lex-scholar-rehberi.md` |
| **İmzalı PSA/JOA emsali, kloz benchmark** 🆕 | **ResourceContracts MCP** | `resourcecontracts-rehberi.md` |
...
**Önce TR.** Bir sorunun Türk hukuku ayağı varsa önce ArthurLegal MCP (`tr_`)'ye git;
yabancı kaynak ek katmandır.

**Kaynak hiyerarşisi — çelişki hâlinde:**
**birincil (mevzuat/içtihat) > emsal sözleşme > doktrin.**
Doktrin veya bir emsal kloz, mevzuatla çelişiyorsa **birincil kaynak üstündür**;
çelişkiyi raporla, doktrin lehine sessizce çözme. Bir emsal sözleşmedeki şart
**bir tarafın müzakere sonucudur**, hukukun emri değil.
...
3. **İcra-edilebilirliği çapraz-kontrol et:**
   ...
   - Uygulanacak hukuk **Türk hukuku** → ArthurLegal MCP (`tr_`) (TBK/TTK).
...
| Kaynak | Etiket |
|---|---|
| Türk mevzuatı | `[ArthurLegal TR — GG.AA.YYYY]` |
| Türk/idari yargı | `[ArthurLegal TR — kurum — Esas/Karar — GG.AA.YYYY]` |
...
- **v1.4.0 (paket sürümü):** MCP sunucu tablosu, 100 saniye kuralı, kaynak
  hiyerarşisi ve üç yeni atıf etiketi eklendi; AZ satırı WebFetch'ten MCP'ye
  taşındı.
