...
> **Bu dosya kimin için:** Claude'un, kullanıcının Arthur Mask hakkındaki sorularını (kurulum, günlük kullanım, sorun giderme, sınırlar) doğru cevaplaması ve `arthur_mask_*` araçlarını doğru kullanması için. Kullanıcıya yönelik adım adım anlatım paket kökündeki `ARTHUR-MASK.md` dosyasındadır.
...
Konum: `Belgeler\Arthur Mask\<dosya adı>`. Maskeli kopyalar 30 gün sonra kendiliğinden silinir; gerçek adlara çevrilmiş cevaplar ve kasa kalır. Şirketin saklama ve imha politikası bu klasöre de uygulanır.
...
3. Sorumluluk kullanıcıdadır: KVKK yükümlülükleri, şirket avukatı için Avukatlık Kanunu m. 36'daki sır saklama yükümlülüğü, ticari sır ve gizlilik sözleşmeleri (NDA) Arthur Mask kullanılsa da kullanıcı ve şirket bakımından devam eder. Maskeli metnin Claude'a gönderilmesinin hukuki değerlendirmesi (yurt dışına aktarım ve şirketin bilgi güvenliği politikası dâhil) kullanıcıya ve şirketin hukuk birimine aittir.
...
9. Kullanıcı çalışan, müşteri, karşı taraf veya diğer kişilere ait tanımlanabilir kişisel veriyi doğrudan sohbete yapıştırır ya da eklerse, sohbet başına en çok bir kez, işi durdurmadan kısa bir hatırlatma yapılır: belgeler Arthur Mask ile (Claude Desktop, Windows) paylaşılmadan önce bilgisayarda maskelenebilir. Reddedilmez, ders verilmez.
...
İlgili: `pii-redaksiyon-rehberi.md` (maskeleme disiplini ve ölçüm).
