...
- **Grup içi compliance prosedürleri** — ana ortak grup politikası
...
```
tr_mevzuat_ara(number="7262", types=["KANUN"])  → Kitle İmha Silahlarının Yayılmasının Finansmanı K.
tr_mevzuat_ara(query="MASAK", types=["KKY"])
tr_mevzuat_ara(number="5549", types=["KANUN"])  → MASAK Suç Gelirlerinin Aklanması K.
tr_mevzuat_ara(query="yaptırım", types=["CB_KARARNAME"], search_in="fulltext")
```

Yabancı yaptırım rejimleri (OFAC, AB, BM, UK) ArthurLegal MCP (`tr_`)'de YOK — doğrudan resmi sitelerden veya screening service'ten çek.
