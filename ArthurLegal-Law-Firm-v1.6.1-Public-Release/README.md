...
**Version:** v1.6.1 · **Date:** 2026-09-04 · **License:** Proprietary — Non-Commercial (see [LICENSE](LICENSE))
**Target environment:** [Claude.ai Projects](https://claude.ai/projects) (web)

> **Multi-jurisdiction legal assistant package for small-to-mid law firms (0–30 staff)**, built on Claude and packaged as a Claude.ai Projects bundle — `SYSTEM_PROMPT.md` + 91 knowledge files + up to 5 MCP connectors, covering **28 jurisdictions**.
>
...
| **ArthurLegal MCP** | MCP — `arthurlegal-mcp.fly.dev/mcp` | None | **Ten jurisdictions behind one connector — 63 tools.** 🇦🇿 Azerbaijani legislation, in-force status verified (`az_`) · 🇦🇹 Austria RIS (`at_`) · 🇩🇪 Germany, federal legislation + six supreme courts (`de_`) · 🇳🇱 Netherlands (`nl_`) · 🇵🇱 Poland (`pl_`) · 🇪🇸 Spain BOE (`es_`) · 🇫🇮 Finland (`fi_`) · 🇮🇪 Ireland (`ie_`) · 10 open-access scholarship indexes incl. **DergiPark, 19 Turkish law journals** (`scholar_`) · 5,125 signed petroleum & mining contracts, 107 countries (`contracts_`) |
